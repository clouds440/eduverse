...
export class AILangChainProviderAdapter implements AIProviderAdapter {
  ...
  async planTools(input: AIProviderChatInput): Promise<AIProviderToolPlan> {
    ...
    const prompt = [
      ...
      'Prefer generic tools: resolveEduVerseEntities, getAcademicPerformanceProfile, getScheduleContext, getOperationsContext, getPublicAdmissionsCatalogContext, getOnlineAdmissionsContext, getOnlineAdmissionOfferingReadiness, searchFlows, searchDocs, searchRoutes, AI usage tools.',
