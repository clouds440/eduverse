...
function planActionLabel(
  ...
  const isCurrentActive =
    activeStatus === AISubscriptionStatus.ACTIVE && activePlan === plan;
...
function PlanGrid({
  ...
  const paidPlans = plans.filter(
    (plan) =>
      plan.plan !== AISubscriptionPlan.NONE &&
      plan.plan !== AISubscriptionPlan.FREE,
  );
    ...
    <section className="rounded-lg border border-border/70 bg-card p-4 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        ...
        {activePlan && (
          <Badge
            variant={
              activePlan === AISubscriptionPlan.NONE ? "secondary" : "success"
            }
            size="md"
          >
            {activePlan}
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {paidPlans.map((plan) => {
          const isCurrentActive =
            activeStatus === AISubscriptionStatus.ACTIVE &&
            activePlan === plan.plan;
          const isDowngrade = planRank(activePlan) > planRank(plan.plan);
          const downgradeExhaustsCredits =
            isDowngrade && usedCredits >= plan.monthlyCredits;
          return (
            <div
              key={plan.plan}
              className="flex min-h-52 flex-col rounded-lg border border-border/70 bg-background/65 p-4"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                    {plan.limitMode} limit
                  </p>
                </div>
                <CopilotIcon className="h-4 w-4 text-primary" aria-hidden="true" />
              </div>
              ...
              <p className="mt-3 flex-1 text-sm font-semibold leading-6 text-muted-foreground">
                {plan.description}
              </p>
              {downgradeExhaustsCredits && (
                <div className="mt-3 flex items-start gap-2 rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-xs font-semibold leading-5 text-warning">
                  ...
                  />
                  <span>
                    You have already used {formatCredits(usedCredits)} credits,
                    so this downgrade may leave no credits available until
                    renewal.
                  </span>
                </div>
              )}
...
export default function AISubscriptionPage() {
  ...
  const { data: orgSettings, isLoading: orgLoading } =
    useSWR<AIOrgSettingsResponse>(
      token && isOrgAdmin ? ["ai-org-settings-subscription", token] : null,
      ([, authToken]) => api.ai.getOrgSettings(authToken as string),
    );
  const { data: personalSettings, isLoading: personalLoading } =
    useSWR<AIPersonalSettingsResponse>(
      token ? ["ai-personal-settings-subscription", token] : null,
      ([, authToken]) => api.ai.getPersonalSubscription(authToken as string),
    );

  const plans = useMemo(
    () => personalSettings?.plans ?? orgSettings?.plans ?? [],
    [orgSettings?.plans, personalSettings?.plans],
  );
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Failed to start organization AI checkout";
  ...
  const startPersonalCheckout = async (plan: AISubscriptionPlan) => {
    ...
    dispatch({
      type: "UI_START_PROCESSING",
      payload: `ai-personal-checkout-${plan}`,
    });
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Failed to start personal AI checkout";
      dispatch({ type: "TOAST_ADD", payload: { message, type: "error" } });
    } finally {
      dispatch({
        type: "UI_STOP_PROCESSING",
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            {isOrgAdmin && (
              <Link
                className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-border bg-surface-raised px-3 py-2 text-xs font-semibold text-foreground shadow-xs transition-colors hover:border-primary/35 hover:bg-muted/70"
                href={user?.id ? settingsPath(user.id, "ai") : "/ai"}
              >
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/70 bg-card p-4 shadow-sm">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-semibold text-muted-foreground">
                  Current plan: {orgSettings.subscription.plan} -{" "}
                  {orgSettings.subscription.status}
                </p>
              </div>
              {orgSettings.subscription.lemonSqueezySubscriptionId && (
                <Button
                  ...
                  variant="secondary"
                  icon={ExternalLink}
                  loadingId="ai-org-portal"
                  onClick={() =>
                    openPortal(AISubscriptionOwnerType.ORGANIZATION)
                  }
                  className="text-xs"
                  px="px-3"
                  py="py-2"
          ...
          <div className="grid gap-3">
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/70 bg-card p-4 shadow-sm">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-semibold text-muted-foreground">
                  Current plan: {personalSettings.subscription.plan} -{" "}
                  {personalSettings.subscription.status}
                </p>
              </div>
              {personalSettings.subscription.lemonSqueezySubscriptionId && (
                <Button
                  ...
                  variant="secondary"
                  icon={ExternalLink}
                  loadingId="ai-personal-portal"
                  onClick={() => openPortal(AISubscriptionOwnerType.USER)}
                  className="text-xs"
                  px="px-3"
                  py="py-2"
                >
        ...
        <section className="rounded-lg border border-border/70 bg-card p-4 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm font-semibold leading-6 text-muted-foreground">
              Organization subscriptions can enable Copilot by role. Personal
              subscriptions only unlock Copilot for the purchasing user and
