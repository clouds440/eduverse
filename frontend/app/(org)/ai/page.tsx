  ...
  return (
    <PageShell className="gap-0 overflow-y-auto pb-8 custom-scrollbar">
      <PageHeader
        ...
        breadcrumbs={[{ label: "Organization" }, { label: "EduVerse Copilot" }]}
        className="mb-2"
      />
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-semibold text-muted-foreground">
                  ...
                  AIUsageSourceType.ORGANIZATION
                    ? "Organization-funded Copilot"
                    : "Personal Copilot"}{" "}
                  -{" "}
                  {entitlement.source.balance.remainingCredits.toLocaleString()}{" "}
                  credits remaining
                </p>
              </div>
              <Badge variant="success" size="md">
                {entitlement.source.subscription.plan}
              </Badge>
            </div>
          </section>
          {isOrgAdmin &&
            entitlement.source.sourceType ===
              AIUsageSourceType.ORGANIZATION && <AIOrgDashboard />}
          {entitlement.source.sourceType === AIUsageSourceType.PERSONAL && (
            <AIPersonalDashboard />
          )}
        </div>
      ) : (
        <section className="rounded-lg border border-warning/30 bg-warning/10 p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 className="text-base font-black text-warning">
                  EduVerse Copilot is ready to unlock
                </h2>
                <p className="mt-1 text-sm font-semibold leading-6 text-warning/85">
                  {entitlement?.message ??
                    "Choose an EduVerse Copilot plan to activate role-aware assistance and usage insights."}
                </p>
              </div>
            </div>
            <Button
              ...
              variant="primary"
              icon={CopilotIcon}
              onClick={() => window.location.assign("/ai/subscription")}
              className="text-xs"
              px="px-4"
              py="py-2.5"
            >
