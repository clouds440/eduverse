...
function Reassign({ cycles, token, dispatch }: { cycles: AcademicCycle[]; token: string; dispatch: ReturnType<typeof useGlobal>['dispatch'] }) {
    ...
    const [attendanceTransferMode, setAttendanceTransferMode] = useState<'PRESERVE_ONLY' | 'PERCENTAGE_ADJUSTMENT'>('PRESERVE_ONLY');
    ...
    const submit = async () => { dispatch({ type: 'UI_START_PROCESSING', payload: 'reassign-students' }); setError(''); try { const result = await api.reassignment.reassignStudents({ sourceType, fromCycleId, toCycleId, fromCohortId: sourceType === 'cohort' ? fromId : undefined, toCohortId: sourceType === 'cohort' ? toId : undefined, fromSectionId: sourceType === 'section' ? fromId : undefined, toSectionId: sourceType === 'section' ? toId : undefined, wasExcluded: sourceType === 'section' ? wasExcluded : undefined, attendanceTransferMode: sourceType === 'section' ? attendanceTransferMode : undefined, transferDate: sourceType === 'section' ? transferDate : undefined, reason: reason || undefined }, token); dispatch({ type: 'TOAST_ADD', payload: { message: `${result.reassigned} students reassigned; ${result.skipped} skipped`, type: 'success' } }); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to reassign students'); } finally { dispatch({ type: 'UI_STOP_PROCESSING', payload: 'reassign-students' }); } };
    return <Workflow title="Reassign students" text="Use this dedicated transfer utility for real cohort or section moves. It closes the source placement historically, creates the destination placement, and preserves academic records.">{error && <ErrorText text={error} />}<Field label="Source type"><CustomSelect value={sourceType} onChange={(value) => { setSourceType(value); setFromId(''); setToId(''); }} options={[{ value: 'cohort', label: 'Cohort offering' }, { value: 'section', label: 'Section transfer' }]} /></Field><CyclePair cycles={cycles} from={fromCycleId} to={toCycleId} onFrom={(id) => { setFromCycleId(id); setFromId(''); }} onTo={(id) => { setToCycleId(id); setToId(''); }} /><div className="grid gap-4 md:grid-cols-2"><Field label={`Source ${sourceType}`}><CustomSelect searchable value={fromId} onChange={setFromId} options={sourceType === 'section' ? (fromSections?.data || []).map(sectionOption) : sourceCohortOfferings} /></Field><Field label={`Destination ${sourceType}`}><CustomSelect searchable value={toId} onChange={setToId} options={sourceType === 'section' ? (toSections?.data || []).map(sectionOption) : targetCohortOfferings} /></Field></div>{sourceType === 'section' && <div className="space-y-4 rounded-md border border-warning/35 bg-warning/10 p-4"><div><p className="text-sm font-black">Section transfer handling</p><p className="mt-1 text-xs font-semibold text-muted-foreground">Source attendance, grades, submissions, and history are preserved. Destination assessments before the transfer date are exempted instead of marked zero.</p></div><div className="grid gap-4 md:grid-cols-2"><Field label="Transfer date"><Input type="date" value={transferDate} onChange={(event) => setTransferDate(event.target.value)} /></Field><Field label="Source transcript treatment"><CustomSelect value={wasExcluded ? 'EXCLUDE' : 'INCLUDE'} onChange={(value) => setWasExcluded(value === 'EXCLUDE')} options={[{ value: 'INCLUDE', label: 'Preserve and include in GPA and merit' }, { value: 'EXCLUDE', label: 'Preserve but exclude from GPA and merit' }]} /></Field><Field label="Attendance handling"><CustomSelect value={attendanceTransferMode} onChange={(value) => setAttendanceTransferMode(value as 'PRESERVE_ONLY' | 'PERCENTAGE_ADJUSTMENT')} options={[{ value: 'PRESERVE_ONLY', label: 'Preserve source attendance only' }, { value: 'PERCENTAGE_ADJUSTMENT', label: 'Create auditable percentage adjustment' }]} /></Field><Field label="Reason"><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Optional transfer reason" /></Field></div>{attendanceTransferMode === 'PERCENTAGE_ADJUSTMENT' && <p className="rounded-md border border-warning/30 bg-background/70 px-3 py-2 text-xs font-semibold text-muted-foreground">Percentage adjustment creates clearly labelled administrative attendance records in previous destination sessions. It does not map exact weekdays or claim the student attended those exact sessions.</p>}</div>}<div className="flex justify-end"><Button loadingId="reassign-students" disabled={!fromCycleId || !toCycleId || !fromId || !toId || (sourceType === 'section' && !transferDate)} onClick={submit}>Reassign students</Button></div></Workflow>;
}
