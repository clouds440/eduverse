...
export default function BuildingsTab() {
  ...
  } = useSWR<PaginatedResponse<Building>>(
  ...
  const openEdit = (building: Building) => {
    ...
    setFormData({
      ...
      departmentIds:
        building.departments?.map((department) => department.id) || [],
    });
    setPendingImage(null);
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (editingBuilding) {
        savedBuilding = await api.org.updateBuilding(
          editingBuilding.id,
          payload,
          token,
        );
        if (pendingImage) {
          await api.org.uploadBuildingImage(
            savedBuilding.id,
            pendingImage,
            token,
          );
        ...
        dispatch({
          type: "TOAST_ADD",
          payload: { message: "Building updated", type: "success" },
        });
      } else {
        ...
        dispatch({
          type: "TOAST_ADD",
          payload: { message: "Building created", type: "success" },
        });
      }
      setModalOpen(false);
      setPendingImage(null);
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: {
          message: apiError.message || "Unable to save building",
          type: "error",
        },
      });
    } finally {
  ...
  const confirmActiveChange = async () => {
    ...
    try {
      await api.org.setBuildingActive(
        activeTarget.id,
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: {
          message: apiError.message || "Unable to update building status",
          type: "error",
        },
      });
    }
        ...
        accessor: (row) => (
          <div className="flex min-w-0 items-center gap-3">
            <div className="relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-md border border-border/70 bg-primary/10 text-primary">
              {row.imageUrl ? (
                <Image
                  src={getPublicUrl(row.imageUrl, row.imageUpdatedAt)}
                  alt={row.name}
                  fill
  ...
  if (error)
    return <ErrorState error={error} onRetry={() => mutateBuildings()} />;
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              value={formData.directionsNote}
              onChange={(event) =>
                setFormData({ ...formData, directionsNote: event.target.value })
              }
              placeholder="Enter from Gate 1 and turn left after reception."
            />
          </div>
          <div className="space-y-2">
