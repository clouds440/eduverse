...
export default function PastRecordsPage() {
    ...
    const filters = { page, limit: pageSize, search: search || undefined, cycleId: cycleId || undefined, departmentId: departmentId || undefined, programId: programId || undefined, cohortId: cohortId || undefined, studentId: studentId || undefined, classification: classification || undefined };
    ...
    const filterPanel = (
        <FilterDrawerGrid>
            ...
            <div><Label>Department</Label><CustomSelect value={departmentId} onChange={(value) => updateQueryParams({ departmentId: value || undefined, programId: undefined, page: 1 })} options={[{ value: '', label: 'All departments' }, ...(options?.departments || []).map((item) => ({ value: item.id, label: item.label }))]} /></div>
            <div><Label>Program</Label><CustomSelect value={programId} onChange={(value) => updateQueryParams({ programId: value || undefined, classification: value ? ProgramClassificationStatus.PROGRAM_MAPPED : classification || undefined, page: 1 })} options={[{ value: '', label: 'All programs' }, ...(options?.programs || []).filter((item) => !departmentId || item.departmentLabel === options?.departments.find((department) => department.id === departmentId)?.label).map((item) => ({ value: item.id, label: item.label }))]} /></div>
