...
function SectionGradeCard({ section }: { section: Section }) {
  ...
  return (
    ...
    >
      <div
        className="flex min-w-0 items-start justify-between gap-3 border-b p-4"
        style={{
          borderColor: `${sectionColor}38`,
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <CourseSectionLabel
              section={section}
              as="h2"
              className="truncate text-base font-black md:text-lg"
            />
      ...
      <div className="grid gap-2 p-3 sm:grid-cols-2 sm:p-4">
        <div className="rounded-md border p-3" style={sectionPanelStyle}>
          ...
          <p
            ...
            style={{ color: sectionColor }}
          >
            All students and assessment marks
          </p>
        </div>
        <div className="rounded-md border p-3" style={sectionPanelStyle}>
          <div
      ...
      <div
        className="flex flex-col gap-2 border-t p-3 sm:flex-row sm:p-4"
        style={{ borderColor: `${sectionColor}2E` }}
      >
        <Link
          href={`/grades/${section.id}`}
...
type UnfinalizedStatusFilter =
  | "ALL"
  | GradeStatus.DRAFT
  | GradeStatus.PUBLISHED;

const UNFINALIZED_STATUS_OPTIONS: {
  value: UnfinalizedStatusFilter;
  label: string;
}[] = [
...
function buildUnfinalizedGradeRows(
  ...
  return assessments.flatMap((assessment, index) => {
    ...
    return grades
      .filter(
        (grade) => grade.status !== GradeStatus.FINALIZED && grade.student,
      )
      .map((grade) => ({
...
function UnfinalizedGradesPanel({
  token,
  canReview,
}: {
  token: string | null;
  ...
  const [statusFilter, setStatusFilter] =
    useState<UnfinalizedStatusFilter>("ALL");
  const [sectionFilter, setSectionFilter] = useState("");
  const [editingRow, setEditingRow] =
    useState<UnfinalizedGradeReviewRow | null>(null);

  const reviewKey =
    token && canReview ? (["unfinalized-grade-review", token] as const) : null;
  const {
    data: reviewRows = [],
    isLoading,
    error,
    mutate,
  } = useSWR<UnfinalizedGradeReviewRow[]>(reviewKey, async () => {
    const assessments = await api.org.getAssessments(token!);
    const gradeLists = await Promise.all(
      assessments.map((assessment) => api.org.getGrades(assessment.id, token!)),
    );
  ...
  const filteredRows = useMemo(() => {
    ...
    return reviewRows.filter((row) => {
      const matchesStatus =
        statusFilter === "ALL" || row.grade.status === statusFilter;
      const matchesSection = !sectionFilter || row.sectionId === sectionFilter;
      const matchesSearch =
        !term ||
        fuzzySearchScore(term, [
  ...
  // Client-side sort state and handler
  ...
  const sortedRows = useMemo(() => {
    ...
    const compare = (
      a: UnfinalizedGradeReviewRow,
      b: UnfinalizedGradeReviewRow,
    ) => {
      const get = (row: UnfinalizedGradeReviewRow) => {
        switch (key) {
          case "student.name":
            return (row.student.user?.name || "").toString().toLowerCase();
          case "marks":
            return Number(row.grade.marksObtained || 0);
          case "status":
            return (row.grade.status || "").toString();
          case "weight":
            return Number(row.weightage || 0);
          default:
            return "";
        }
      };
    ...
    async (row: UnfinalizedGradeReviewRow) => {
      ...
      try {
        ...
        dispatch({
          type: "TOAST_ADD",
          payload: {
            message: `Finalized ${row.student.user.name}'s grade.`,
            type: "success",
          },
        });
        mutate();
  ...
  return (
    <ResourcePanel className="overflow-y-auto">
        ...
        <StatusBanner
          ...
          description={
            <>
              ...
              <DocsLink href="/docs/gradebook#grades-page">
                Read gradebook rules
              </DocsLink>
            </>
          }
          dismissible={true}
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                ...
                accessor: (row) => (
                  <Badge
                    variant={getGradeStatusVariant(row.grade.status)}
                    size="sm"
                  >
                    {row.grade.status === GradeStatus.DRAFT
                      ? "Draft"
                      : "Published"}
                  </Badge>
                ),
            ...
            emptyDescription={
              reviewRows.length === 0
                ? "All entered grades are finalized, or no grades have been entered yet."
      ...
      <Modal
        ...
        subtitle={
          editingRow
            ? `${editingRow.student.user.name} - ${editingRow.assessmentTitle}`
            : ""
        }
        maxWidth="max-w-xl"
      >
...
export default function GradesPage() {
  ...
  const sectionsKey =
    token && user
      ...
      : null;
  const { data: sectionsData, isLoading } = useSWR<{ data: Section[] }>(
    sectionsKey,
  );
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description={
          <>
            Choose a section to review every student grade or open its control
            panel.{" "}
            <DocsLink href="/docs/gradebook#grades-page">
              Read grade docs
            </DocsLink>
      ...
      {canReviewUnfinalized ? (
        <UnfinalizedGradesPanel
          token={token}
          canReview={Boolean(canReviewUnfinalized)}
        />
      ) : isLoading ? (
        <>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4">
            {filteredSections.length === 0 ? (
              <EmptyState
                ...
                description={
                  searchTerm
                    ? "Try a different section or course search."
                    : "Sections will appear here when they are available for grading."
                }
                className="min-h-72"
              />
            ) : (
