...
import { AcademicEvent, AcademicEventType, Department, DepartmentScopeType, PaginatedResponse, Role } from '@/types';
...
const DEFAULT_EVENT_BANNER = '/assets/eduverse.png';
...
function getBannerSrc(event: AcademicEvent) {
    return event.bannerUrl ? getPublicUrl(event.bannerUrl, event.bannerUpdatedAt) : DEFAULT_EVENT_BANNER;
}
...
export default function AcademicCalendarPage() {
    ...
    const { data, isLoading, error, mutate } = useSWR<PaginatedResponse<AcademicEvent>>(
    ...
    const activeFilters: ActiveFilter[] = [
        ...
        ...(typeFilter ? [{ key: 'type', label: 'Type', value: getTypeLabel(typeFilter as AcademicEventType), onRemove: () => updateQueryParams({ type: undefined }) }] : []),
    ...
    return (
        <PageShell>
            <PageHeader
                ...
                icon={Calendar}
                meta={<Badge variant="neutral" size="sm">{data?.totalRecords || 0} active</Badge>}
                ...
                actions={(
                    <PageControls
                        ...
                        leading={<SearchBar value={searchTerm} onChange={(value) => updateQueryParams({ search: value || undefined })} placeholder="Search events..." mobileMode="expandable" />}
                        actions={canManage ? (
                            <Link
                ...
                {isLoading ? (
                    <div className="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3">
                        {Array.from({ length: 6 }).map((_, index) => (
                            <div key={index} className="h-80 animate-pulse rounded-lg border border-border bg-muted/30" />
                        ))}
                    </div>
                ) : data?.data.length ? (
                    <div className="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3">
                        {data.data.map((event) => (
                            <article key={event.id} className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
                                ...
                                <div className="space-y-3 p-4">
                                    <div className="flex flex-wrap items-center gap-2">
                                        <Badge variant="neutral" size="sm">{getTypeLabel(event.type)}</Badge>
                                        <Badge variant="info" size="sm">{formatDateRange(event)}</Badge>
                                    </div>
                                    <div>
                                        <h2 className="line-clamp-2 text-base font-black text-foreground">{event.title}</h2>
                                        {event.description && <p className="mt-1 line-clamp-3 text-sm font-medium text-muted-foreground">{event.description}</p>}
                                    </div>
                                    <div className="flex items-center justify-between gap-3 border-t border-border pt-3 text-xs font-bold text-muted-foreground">
                                        <span className="truncate">{getDepartmentSummary(event)}</span>
                                        <span className="shrink-0">{event.isFullDay ? 'Full day' : `${event.startTime} - ${event.endTime}`}</span>
                                    </div>
                                </div>
                            </article>
                        ))}
                    ...
                    <div className="p-8">
                        <EmptyState
                            ...
                            description={activeFilters.length ? 'Adjust the filters to broaden the calendar view.' : 'Active academic events will appear here.'}
                        />
                    </div>
                )}
            </ResourcePanel>
