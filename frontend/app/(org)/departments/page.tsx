...
export default function DepartmentsPage() {
  ...
  } = useSWR<PaginatedResponse<Department>>(
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (editingDepartment) {
        ...
        dispatch({
          type: "TOAST_ADD",
          payload: { message: "Department updated", type: "success" },
        });
      } else {
        await api.org.createDepartment(payload, token);
        dispatch({
          type: "TOAST_ADD",
          payload: { message: "Department created", type: "success" },
        });
      }
      setModalOpen(false);
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: {
          message: apiError.message || "Unable to save department",
          type: "error",
        },
      });
    } finally {
  ...
  const confirmActiveChange = async () => {
    ...
    try {
      await api.org.setDepartmentActive(
        activeTarget.id,
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: {
          message: apiError.message || "Unable to update department status",
          type: "error",
        },
      });
    }
    ...
    () => [
      ...
      },
      {
        ...
        accessor: (row) => {
          ...
          const visible = buildings.slice(0, 2);
          const remaining = buildings.length - visible.length;
              ...
              {buildings.length ? (
                <>
                  {visible.map((building) => (
                    <Badge key={building.id} variant="neutral" size="sm">
      ...
      },
      {
        ...
        accessor: (row) => (
          <TableActions
            ...
            extraActions={
              canChangeDepartmentStatus
                ? [
                    {
                      variant: row.isActive ? "suspend" : "restore",
                      title: row.isActive
                        ? "Deactivate Department - hide it from active setup filters"
                        : "Activate Department - make it available in setup filters",
  ...
  if (error)
    return <ErrorState error={error} onRetry={() => mutateDepartments()} />;
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Optional notes for this department"
            />
          </div>
        </div>
      </ModalForm>
