...
const appLinkTerms: AppLinkTerm[] = [
  ...
  { phrase: "Import students CSV", href: "/users/students" },
  { phrase: "Import teachers CSV", href: "/users/teachers" },
  { phrase: "Import courses CSV", href: "/courses" },
  { phrase: "Import sections CSV", href: "/sections" },
  { phrase: "Import guardians CSV", href: "/users/guardians" },
  ...
  { phrase: "Academic Calendar", href: "/academic-calender" },
  { phrase: "academic calendar items", href: "/academic-calender" },
  { phrase: "Buildings and Rooms", href: "/buildings-and-rooms" },
  { phrase: "Building and Room", href: "/buildings-and-rooms" },
  { phrase: "GPA Policies", href: "/settings?tab=gpa-policies" },
  { phrase: "GPA Policy", href: "/settings?tab=gpa-policies" },
  { phrase: "organization default policy", href: "/settings?tab=gpa-policies" },
  { phrase: "Evaluation windows", href: "/evaluations" },
  { phrase: "Evaluation window", href: "/evaluations" },
  { phrase: "Preference Windows", href: "/preference-windows" },
  { phrase: "Preference Window", href: "/preference-windows" },
  { phrase: "preference windows", href: "/preference-windows" },
  { phrase: "preference window", href: "/preference-windows" },
  ...
  { phrase: "student edit form", href: "/users/students" },
  { phrase: "student portal", href: "/users/students" },
  { phrase: "guardian portal", href: "/guardian" },
  { phrase: "guardian account", href: "/users/guardians" },
  { phrase: "linked-student records", href: "/guardian" },
  { phrase: "Finance Managers", href: "/users/finance-managers" },
  { phrase: "Finance Manager", href: "/users/finance-managers" },
  { phrase: "Sub Admins", href: "/users/sub-admins" },
  { phrase: "Sub Admin", href: "/users/sub-admins" },
...
const appLinkPattern = new RegExp(
  `\\b(${appLinkTerms.map((term) => escapeRegExp(term.phrase)).join("|")})\\b`,
  "gi",
);
  ...
  return (
    <article className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <header className="mb-8 border-b border-border pb-8">
        ...
        <div className="mt-5 hidden flex-wrap gap-2 sm:flex">
          {page.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-md border border-border bg-muted/45 px-2.5 py-1 text-xs font-bold text-muted-foreground"
            >
              #{tag}
            </span>
          ))}
      ...
      <nav
        ...
        className="mb-8 hidden rounded-lg border border-border bg-card p-4 lg:block"
      >
            ...
            {relatedPages.map((doc) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-black text-foreground group-hover:text-primary">
                      {doc.title}
                    </h3>
...
function DocSectionView({
  ...
  const HeadingTag: "h2" | "h3" | "h4" =
    depth === 2 ? "h2" : depth === 3 ? "h3" : "h4";

  return (
    <section
      id={section.id}
      className="scroll-mt-24 rounded-lg transition-colors target:bg-primary/5 target:ring-1 target:ring-primary/20"
    >
      <div className="px-1 py-1">
        ...
        {section.summary && (
          <p className="mt-2 text-sm font-semibold leading-relaxed text-muted-foreground">
            <LinkedText text={section.summary} linkedPhrases={linkedPhrases} />
          </p>
        )}
        {section.tags?.length ? (
  ...
  if (block.type === "paragraph") {
    return (
      <p className="text-sm font-medium leading-7 text-muted-foreground">
        <LinkedText text={block.text} linkedPhrases={linkedPhrases} />
      </p>
    );
  }
          ...
          <thead>
            <tr className="border-b border-border bg-muted/50">
              {block.headers.map((header) => (
                <th
                  key={header}
                  className="px-4 py-2.5 text-left text-xs font-black uppercase tracking-wider text-foreground"
                >
      ...
      <ul className="space-y-2">
        {block.items.map((item) => (
          ...
          >
            ...
            />
            <span>
              <LinkedText text={item} linkedPhrases={linkedPhrases} />
            </span>
          </li>
        ))}
      </ul>
