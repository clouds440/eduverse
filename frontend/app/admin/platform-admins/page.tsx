...
export default function PlatformAdminsPage() {
  ...
  const { getNumberParam, getStringParam, updateQueryParams } =
    useUrlQueryState();
  ...
  const {
    ...
    isLoading: fetching,
    mutate: retryAdmins,
  } = useSWR<PaginatedResponse<PlatformAdmin>>(adminsKey);

  const [isAdminModalOpen, setIsAdminModalOpen] = useState(false);
  const [adminModalMode, setAdminModalMode] = useState<"CREATE" | "EDIT">(
    "CREATE",
  );
  const [operatingAdmin, setOperatingAdmin] = useState<PlatformAdmin | null>(
    null,
  );
  ...
  const handleAdminSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (adminModalMode === "CREATE") {
        ...
        await api.admin.updatePlatformAdmin(
          operatingAdmin.id,
          {
  ...
  const handleConfirmDelete = async (e: React.FormEvent) => {
    ...
    try {
      dispatch({
        type: "UI_START_PROCESSING",
        payload: `platform-admin-delete-${operatingAdmin.id}`,
      });
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: {
          message: `${operatingAdmin.name} deleted successfully`,
          type: "success",
        },
      });
        ...
        accessor: (row) => (
          <div className="flex items-start gap-4 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <h4 className="text-sm font-black text-foreground leading-tight wrap-break-word">
                {row.name}
              </h4>
              <span className="text-[10px] font-bold text-muted-foreground tracking-widest block mt-0.5">
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        icon={Users}
        breadcrumbs={[{ label: "Admin" }, { label: "Platform Admins" }]}
        meta={
          paginatedData?.totalRecords !== undefined ? (
      ...
      <ResourcePanel>
        <div className="flex-1 min-h-0">
          <DataTable
            ...
            emptyDescription={
              searchQuery
                ? "Clear or adjust the search to broaden the result set."
                : "Create a platform admin to delegate operations safely."
            }
            mobileDetailLimit={3}
          />
        </div>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setAdminFormData((prev) => ({ ...prev, phone: e.target.value }))
              }
      ...
      >
        <div>
          <p className="text-sm font-medium text-foreground">
            ...
            <strong>{operatingAdmin?.name}</strong> from Platform Admins?
          </p>
