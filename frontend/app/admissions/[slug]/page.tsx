...
import { ArrowLeft, BookOpen, CalendarRange, FileText } from 'lucide-react';
                        ...
                        {data.programOfferings.length === 0 ? (
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                                {data.programOfferings.map((offering) => {
                                    const requirements = offering.onlineAdmissionDocumentRequirements || [];
                                    return (
                                        <Link
                                            key={offering.id}
                                            href={`/admissions/${data.slug}/${offering.id}`}
                                        ...
                                        >
                                            <div className="flex min-w-0 items-start justify-between gap-3">
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="mt-1 text-sm font-semibold text-muted-foreground">{offering.program.admissionsLabel || offering.program.department?.name || 'Program admission'}</p>
                                                </div>
                                            ...
                                            <div className="mt-4 grid gap-2 text-sm font-semibold text-muted-foreground sm:grid-cols-2">
                                                <span className="inline-flex items-center gap-2"><CalendarRange className="h-4 w-4" />{offering.academicCycle.code}</span>
