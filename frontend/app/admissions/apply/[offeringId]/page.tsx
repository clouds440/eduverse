...
export default function ProviderNeutralAdmissionApplicationPage() {
    ...
    const { data: offering, error, isLoading } = useSWR<PublicOnlineAdmissionOffering>(
        ...
        () => api.publicOnlineAdmissions.getOffering(decodeURIComponent(offeringId)),
    );
                    ...
                    <>
                        <header className="rounded-lg border border-border/70 bg-card p-5 shadow-sm">
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                <Badge variant="neutral" size="sm">{offering.intakeName}</Badge>
                            </div>
                            <h1 className="mt-3 text-3xl font-black tracking-tight">{offering.program.name}</h1>
                            <p className="mt-1 text-sm font-semibold text-muted-foreground">{offering.provider.displayName}</p>
                        ...
                        <section className="grid gap-3 md:grid-cols-3">
                            {Boolean(offering.fees?.length) && <DisclosurePanel icon={BadgeDollarSign} title="Fees">{offering.fees!.map((fee) => <p key={fee.id || fee.label} className="border-b border-border/60 py-2 text-sm font-semibold last:border-0">{fee.label}{fee.amount ? `: ${fee.currencyCode} ${fee.amount}` : ''}{fee.frequency ? ` - ${fee.frequency}` : ''}</p>)}</DisclosurePanel>}
                            {Boolean(offering.admissionRequirements?.length) && <DisclosurePanel icon={ShieldCheck} title="Eligibility">{offering.admissionRequirements!.map((item) => <p key={item.id || item.label} className="border-b border-border/60 py-2 text-sm font-semibold last:border-0">{item.label}{item.isRequired ? ' *' : ''}</p>)}</DisclosurePanel>}
                            {Boolean(offering.fundingOptions?.length) && <DisclosurePanel icon={Gift} title="Funding">{offering.fundingOptions!.map((item) => <p key={item.id || item.title} className="border-b border-border/60 py-2 text-sm font-semibold last:border-0">{item.title}{item.amountSummary ? `: ${item.amountSummary}` : ''}</p>)}</DisclosurePanel>}
                        </section>
                        ...
                        <form onSubmit={handleSubmit} className="rounded-lg border border-border/70 bg-card p-5 shadow-sm">
                            ...
                            {requirements.length > 0 && (
                                <div className="mt-5 border-t border-border/70 pt-5">
                                    <div className="flex items-center gap-2"><Paperclip className="h-5 w-5 text-primary" aria-hidden="true" /><h2 className="text-base font-black">Upload documents</h2></div>
                                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                                        {requirements.map((item) => (
                                            <label key={item.id} className="space-y-2 rounded-md border border-border/70 bg-muted/20 p-3 text-sm font-bold">
                                                ...
                                                <input type="file" required={item.isRequired} multiple={item.maxFileCount > 1} accept={[...item.acceptedMimeTypes, ...item.acceptedExtensions].join(',') || undefined} onChange={(event) => setDocuments((current) => ({ ...current, [item.id]: Array.from(event.target.files || []).slice(0, item.maxFileCount) }))} className="block w-full text-sm font-semibold text-muted-foreground file:mr-3 file:rounded-md file:border-0 file:bg-primary file:px-3 file:py-2 file:text-sm file:font-bold file:text-primary-foreground" />
                                                {item.requiresExpiryDate && <input type="date" required value={documentExpiryDates[item.id] || ''} onChange={(event) => setDocumentExpiryDates((current) => ({ ...current, [item.id]: event.target.value }))} className="block h-10 w-full rounded-md border border-border bg-background px-3 text-sm" />}
                            ...
                            {offering.applicationForm.consentText && <label className="mt-5 flex items-start gap-3 border-t border-border/70 pt-5 text-sm font-semibold"><input type="checkbox" required checked={consentAccepted} onChange={(event) => setConsentAccepted(event.target.checked)} className="mt-0.5 h-4 w-4 accent-primary" /><span>{offering.applicationForm.consentText}</span></label>}
                            <div className="mt-5"><CapVerification purpose="ONLINE_ADMISSION" onChange={handleVerificationChange} resetKey={verificationResetKey} disabled={isSubmitting} /></div>
                            <div className="mt-5 flex justify-end"><Button type="submit" icon={Send} isLoading={isSubmitting} loadingText="Submitting" disabled={!captchaToken}>Submit application</Button></div>
                        </form>
                    </>
                )}
            </div>
...
function DisclosurePanel({ icon: Icon, title, children }: { icon: React.ElementType<{ className?: string }>; title: string; children: React.ReactNode }) {
    return (
        <div className="rounded-lg border border-border/70 bg-card p-4 shadow-sm">
            <div className="flex items-center gap-2"><Icon className="h-5 w-5 text-primary" aria-hidden="true" /><h2 className="text-base font-black">{title}</h2></div>
