...
import { canUseDirectMessageBlock, getDirectChatTarget } from '@/lib/communicationOptions';
...
export function getDmBlockAction(chat: Chat, currentUser: User): DmBlockAction | null {
    ...
    return {
        ...
        confirmTitle: isBlockedByMe ? `Unblock DMs from ${targetName}?` : `Block DMs from ${targetName}?`,
...
export async function unblockDirectMessages(targetUserId: string, token: string) {
