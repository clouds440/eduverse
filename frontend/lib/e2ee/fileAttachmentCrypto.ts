...
export async function prepareEncryptedFileUpload({
    ...
    if (!clientDeviceId) {
        throw new E2EEError('NO_TRUSTED_DEVICE', 'This browser is not ready for secure attachments.');
    }

    const localKeys = await getLocalTrustedDeviceKeys(currentUserId, clientDeviceId);
    if (!localKeys) {
        throw new E2EEError('NO_TRUSTED_DEVICE', 'Trust this browser before sending secure attachments.');
    }
    ...
    if (deviceRecipients.length === 0) {
        throw new E2EEError('NO_TRUSTED_DEVICE', 'No approved browsers are available for this attachment.');
    }
    ...
    const keyEnvelopes = await Promise.all(deviceRecipients.map(async ({ userId, device }) => {
    ...
    return {
        file: new File([bytesToBlobPart(ciphertext)], 'encrypted-attachment.bin', { type: 'application/octet-stream' }),
        encryptedContent: {
...
export async function downloadEncryptedAttachment(fileId: string, token: string) {
    const metadata = await api.files.getMetadata(fileId, token);
    const encryptedContent = metadata.encryptedContent as EncryptedMailContent | null | undefined;
    ...
    const parsedMetadata = JSON.parse(plaintextMetadata) as { filename?: string; mimeType?: string };
    ...
    const associatedData = encryptedContent.associatedData as Record<string, unknown> | null | undefined;
    const fileNonce = typeof associatedData?.fileNonce === 'string' ? associatedData.fileNonce : null;
    if (!fileNonce) {
        throw new E2EEError('CORRUPT_CIPHERTEXT', 'Encrypted file metadata is missing its nonce.');
    }
    ...
    const decryptedBytes = sodium.crypto_aead_xchacha20poly1305_ietf_decrypt(
        null,
        encryptedBytes,
        encodeAssociatedData(encryptedContent.associatedData as JsonValue | undefined),
        decodeBase64(sodium, fileNonce),
        decodeBase64(sodium, fileKey),
    );
    const blob = new Blob([bytesToBlobPart(decryptedBytes)], { type: parsedMetadata.mimeType || 'application/octet-stream' });
    const blobUrl = window.URL.createObjectURL(blob);
    const anchor = document.createElement('a');
    anchor.href = blobUrl;
    anchor.download = parsedMetadata.filename || metadata.filename || 'attachment';
...
async function unwrapFileKey(encryptedContent: EncryptedMailContent, token: string) {
    const clientDeviceId = getDeviceId();
    if (!clientDeviceId) {
        throw new E2EEError('NO_TRUSTED_DEVICE', 'This browser is not ready for secure attachments.');
    }
    ...
    if (!localKeys || !currentDevice) {
        throw new E2EEError('NO_TRUSTED_DEVICE', 'This browser is not approved for secure attachments.');
    }
    ...
    if (!envelope || !senderPublicKey) {
        throw new E2EEError('NO_KEY_ENVELOPE', "This attachment can't be opened here.");
    }
