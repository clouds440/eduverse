...
const COMMON_ITEMS: SidebarItemConfig[] = [
    ...
    },
    {
        ...
        icon: MessageSquare,
        roles: 'all',
        badge: (ctx) => (ctx.unreadChats && ctx.unreadChats > 0 ? `${ctx.unreadChats}` : undefined),
    },
    {
...
const STUDENT_ITEMS: SidebarItemConfig[] = [
    {
        id: 'COURSES',
        label: 'My Courses',
        href: (ctx) => (ctx.userId ? studentPortalPath(ctx.userId, 'courses') : null),
        icon: Book,
        roles: [Role.STUDENT],
        hiddenWhen: requiresUserId,
    },
    {
        id: 'ASSESSMENTS',
        label: 'Assessments',
        href: (ctx) => (ctx.userId ? studentPortalPath(ctx.userId, 'assessments') : null),
        icon: BookOpen,
        roles: [Role.STUDENT],
        hiddenWhen: requiresUserId,
    },
    {
        id: 'GRADES',
        label: 'Grades',
        href: (ctx) => (ctx.userId ? studentPortalPath(ctx.userId, 'grades') : null),
        icon: Trophy,
        roles: [Role.STUDENT],
        hiddenWhen: requiresUserId,
    },
    {
        id: 'ATTENDANCE',
        label: 'Attendance',
        href: (ctx) => (ctx.userId ? studentPortalPath(ctx.userId, 'attendance') : null),
        icon: CheckCircle,
        roles: [Role.STUDENT],
        hiddenWhen: requiresUserId,
    },
    {
        id: 'EVALUATIONS',
        label: 'Evaluations',
        href: (ctx) => (ctx.userId ? studentPortalPath(ctx.userId, 'evaluations') : null),
        icon: ClipboardList,
        roles: [Role.STUDENT],
        hiddenWhen: requiresUserId,
    },
    ...
    },
    {
        id: 'PROFILE',
        label: 'Profile Settings',
        href: (ctx) => (ctx.userId ? studentPortalPath(ctx.userId, 'profile') : null),
        icon: Settings,
        roles: [Role.STUDENT],
        hiddenWhen: requiresUserId,
    },
...
function roleCanSee(item: SidebarItemConfig, role: Role | null) {
    const isIncluded = item.roles === 'all' || (role !== null && item.roles.includes(role));
...
function resolveSidebarItem(item: SidebarItemConfig, ctx: SidebarContext): SidebarLink | null {
...
export function getSidebarLinksForRole(role: Role | null, options?: GetSidebarLinksForRoleOptions): SidebarLink[];
...
export function getSidebarLinksForRole(
    ...
    return SIDEBAR_ITEMS
        .filter((item) => isSidebarItemVisible(item, ctx))
