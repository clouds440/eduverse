...
export function AICopilotPanel() {
  ...
  const startResize = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      const nextWidth = Math.min(Math.max(window.innerWidth - moveEvent.clientX, 420), 760);
      ...
      >
        {dockedActive && (
          <div
            ...
            className="absolute left-0 top-0 z-10 h-full w-2 cursor-col-resize touch-none bg-transparent transition-colors hover:bg-primary/15"
          />
        )}
          ...
          <div className="flex shrink-0 items-center gap-1">
            {isDesktop && dockHostAvailable && (
              <Button
                ...
                title={dockedActive ? "Use floating Copilot" : "Dock Copilot to the right"}
                aria-label={dockedActive ? "Use floating Copilot" : "Dock Copilot to the right"}
              />
            )}
            <Button
              type="button"
        ...
        {historyOpen && (
          <div className="shrink-0 border-b border-border/70 bg-card/70 p-3">
            <div className="mb-2 flex items-center justify-between gap-3">
              <p className="text-xs font-black uppercase tracking-widest text-muted-foreground">Previous Chats</p>
              <button
                type="button"
                onClick={() => {
                  resetConversation();
            ...
            <div className="grid max-h-52 gap-2 overflow-y-auto custom-scrollbar">
              {conversationsLoading ? (
                <div className="rounded-lg border border-border/70 bg-background/70 p-3 text-sm font-semibold text-muted-foreground">Loading conversations...</div>
              ) : conversations.length === 0 ? (
                <div className="rounded-lg border border-border/70 bg-background/70 p-3 text-sm font-semibold text-muted-foreground">No previous Copilot chats yet.</div>
                    ...
                    {isEditing ? (
                      ...
                      >
                        ...
                        <p className="mt-0.5 text-xs font-semibold text-muted-foreground">
                          ...
                          {conversation.creditTotal > 0 ? ` - ${conversation.creditTotal.toLocaleString()} credits` : ""}
                        </p>
                      </button>
                    )}
                    <button
                      type="button"
                      onClick={() => isEditing ? void saveTitle() : startEditTitle(conversation.id, conversation.title)}
                      className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                      aria-label={isEditing ? "Save chat title" : "Edit chat title"}
                    >
