...
export function AISuggestedPrompts({ suggestions, onSelect, disabled }: AISuggestedPromptsProps) {
    ...
    return (
        <div className="flex min-w-0 items-center gap-1.5">
            ...
            <div
                ref={scrollRef}
                className="flex min-w-0 flex-1 gap-2 overflow-x-auto scroll-smooth py-0.5 scrollbar-none"
            >
                {visibleSuggestions.map((suggestion) => (
                    <button
                        ...
                        className={cn(
                            'shrink-0 max-w-[min(78vw,320px)] rounded-full border border-border/70 bg-background/80 px-3 py-1.5 text-left text-xs font-bold leading-5 text-foreground shadow-xs transition-colors',
