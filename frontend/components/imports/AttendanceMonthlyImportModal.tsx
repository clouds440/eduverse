...
import { AttendanceImportTargetMode, ImportConfirmResult, ImportValidationResult, InvalidImportRow } from '@/types';
        ...
        >
            <div className="space-y-4">
                ...
                {validation?.validRows.length ? (
                    <Preview title="Valid rows" rows={validation.validRows.map((row) => ({ rowNumber: row.rowNumber, detail: `${row.raw.name || ''} (${row.raw.rollNumber || ''})` }))} />
                ) : null}
                {invalidRows.length > 0 && (
                    <Preview danger title="Invalid or failed rows" rows={invalidRows.map((row: InvalidImportRow) => ({ rowNumber: row.rowNumber, detail: formatImportErrors(row.errors) }))} />
                )}
            </div>
        </Modal>
    );
...
function Metric({ label, value }: { label: string; value: number }) {
    return (
        <div className="rounded-lg border border-border/70 bg-background/60 p-3">
            ...
            <p className="mt-1 font-mono text-xl font-black text-foreground">{value}</p>
        </div>
    );
}

function Preview({ title, rows, danger }: { title: string; rows: { rowNumber: number; detail: string }[]; danger?: boolean }) {
    return (
        <section className="overflow-hidden rounded-lg border border-border/70">
            ...
            <div className="max-h-64 overflow-auto custom-scrollbar">
                {rows.slice(0, 120).map((row) => (
                    <div key={`${title}-${row.rowNumber}`} className="grid grid-cols-[90px_1fr] border-b border-border/50 px-3 py-2 text-xs last:border-b-0">
                        <span className="font-mono font-black text-muted-foreground">Row {row.rowNumber}</span>
                        <span className={danger ? 'font-semibold text-danger' : 'font-semibold text-foreground/80'}>{row.detail || '-'}</span>
                    </div>
                ))}
            </div>
        </section>
