...
import {
  ...
  MailPriorityBadge,
  useMailRowClassName,
} from "@/components/mail/MailStatusBadge";
...
export function MailPage({
  ...
  const [manualSelectedMailId, setManualSelectedMailId] = useState<
    string | null
  >(null);
  ...
  const handleMailClick = (mail: MailItem) => {
    ...
    try {
      ...
      const notif = items.find(
        (n) =>
          n.metadata?.mailId === mail.id ||
          (n.actionUrl && n.actionUrl.includes(mail.id)) ||
          n.metadata?.entityId === mail.id,
      );
      if (notif && token) {
  ...
  const statusOptions = [
    ...
    {
      value: MailStatus.OPEN,
      label: "Open",
      badge: state.stats.mail?.countsByStatus?.[MailStatus.OPEN],
      icon: Clock,
      iconClassName: "text-info",
    },
    {
      value: MailStatus.IN_PROGRESS,
      label: "In Progress",
      badge: state.stats.mail?.countsByStatus?.[MailStatus.IN_PROGRESS],
      icon: ArrowUpRight,
      iconClassName: "text-warning",
    },
    {
      value: MailStatus.AWAITING_RESPONSE,
      label: "Awaiting Response",
      badge: state.stats.mail?.countsByStatus?.[MailStatus.AWAITING_RESPONSE],
      icon: MessageSquare,
      iconClassName: "text-primary/80",
    },
    {
      value: MailStatus.RESOLVED,
      label: "Resolved",
      badge: state.stats.mail?.countsByStatus?.[MailStatus.RESOLVED],
      icon: CheckCircle2,
      iconClassName: "text-success",
    },
    {
      value: MailStatus.CLOSED,
      label: "Closed",
      badge: state.stats.mail?.countsByStatus?.[MailStatus.CLOSED],
      icon: XCircle,
      iconClassName: "text-muted-foreground",
    },
  ];

  const statusFilterLabel = statusOptions.find(
  ...
  const directionFilterLabel = directionOptions.find(
  ...
  const columns = useMemo<Column<MailItem>[]>(
    () => [
              ...
              <>
                <div className="flex -space-x-2 mr-1">
                  {row.assignees.slice(0, 2).map((a) => (
                    <div
                      key={a.id}
                      className="w-7 h-7 border-2 border-border rounded-full bg-card/5 shadow-sm"
                    >
                      <BrandIcon
                        variant="user"
                        size="sm"
      ...
      },
      {
        ...
        badge: true,
        accessor: (row: MailItem) => (
          <MailPriorityBadge priority={row.priority} />
        ),
      },
      {
        header: "Messages",
      ...
      <PageHeader
        ...
        description={
          <>
            ...
            one inbox. <DocsLink href="/docs/mail">Read mail docs</DocsLink>
            <Badge variant="secondary" className="text-xs font-black" size="sm">
          ...
          <DataTable
            ...
            emptyDescription={
              searchQuery || activeFilters.length > 0
                ...
                : "Create a new mail thread to start a conversation."
            }
            mobileDetailLimit={3}
          />
        </div>
