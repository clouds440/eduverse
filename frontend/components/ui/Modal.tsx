...
export function ModalOverlay({
  ...
  useEffect(() => {
    ...
    const frameId = window.requestAnimationFrame(() => {
      ...
      const firstFocusable =
        modalNode.querySelector<HTMLElement>(FOCUSABLE_SELECTOR);
  ...
  return createPortal(
    <div
      className={cn(
        "fixed inset-0 z-200 flex overflow-y-auto bg-(--app-surface-overlay) p-2 backdrop-blur-sm transition-opacity duration-200 sm:p-4 md:p-6",
        mobileMode === "sheet"
    ...
    >
      <div
        ...
        className={cn(
          ...
          mobileMode === "full" &&
            "h-dvh max-h-dvh rounded-none sm:h-auto sm:max-h-[calc(100dvh-2rem)] sm:rounded-lg",
          maxWidth,
          className,
        )}
      >
...
export function Modal({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative flex-1 overflow-y-auto px-4 py-4 custom-scrollbar sm:px-5 sm:py-5",
          bodyClassName,
        )}
      >
        {children}
