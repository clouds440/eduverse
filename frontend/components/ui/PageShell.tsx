...
export function RouteBreadcrumbs({
  ...
  const resolvedBreadcrumbs =
    breadcrumbs ?? getRouteOrientation(pathname || "/").breadcrumbs;
...
function getPageScrollElement(header: HTMLElement): HTMLElement | null {
  ...
  const dashboardScrollContainer = header.closest<HTMLElement>(
    '[data-dashboard-scroll-container="true"]',
  );
  if (
    dashboardScrollContainer &&
    hasVerticalScrollBehavior(dashboardScrollContainer)
  ) {
...
function isPageScrollTarget(header: HTMLElement, element: HTMLElement | null) {
  ...
  if (dashboardScrollContainer && !dashboardScrollContainer.contains(element))
    return false;
...
function getPageScrollState(
  ...
  if (isPageScrollTarget(header, preferredTarget || null)) {
    ...
    maxScrollRange = Math.max(
      maxScrollRange,
      getScrollableRange(preferredTarget as HTMLElement),
    );
  }
...
function usePageScrollVisibility(
  ...
  useEffect(() => {
    ...
    const readScrollState = () =>
      getPageScrollState(element, activeScrollTarget);
    ...
    const updateVisibility = () => {
      ...
      if (
        scrollRange < PAGE_TABS_MIN_SCROLL_RANGE ||
        scrollTop <= PAGE_TABS_SHOW_SCROLL_TOP
      ) {
        if (!isVisibleRef.current) setVisible(true);
      } else if (
        isVisibleRef.current &&
        scrollTop > PAGE_TABS_HIDE_SCROLL_TOP &&
        delta > PAGE_TABS_HIDE_SCROLL_DELTA
      ) {
        setVisible(false);
      } else if (
        !isVisibleRef.current &&
        delta < -PAGE_TABS_SHOW_SCROLL_DELTA
      ) {
        setVisible(true);
      }
    ...
    const scheduleUpdate = (event?: Event) => {
      ...
      if (
        ...
        isPageScrollTarget(element, target) &&
      ...
      >
        ...
        >
          {Icon && (
            ...
            >
              {renderIcon(Icon, cn(isCompact ? "h-4 w-4" : "h-5 w-5"))}
            </div>
          )}
          <div
            className={cn(
              "min-w-0",
              isCompact && "flex min-h-7 flex-1 items-center",
            )}
          >
            {!isCompact && (
