...
const SidebarNavLink = React.memo(function SidebarNavLink({
  ...
  return (
      ...
      className={`
                ...
                ${
                  isActive
                    ...
                    : "text-(--sidebar-subtle) hover:bg-(--sidebar-hover) hover:text-(--sidebar-text)"
                }
    ...
    >
      ...
      <Icon
        className={`w-5 h-5 shrink-0 transition-transform ${isActive ? "scale-105 text-primary-foreground" : "text-primary/80 group-hover:scale-105"}`}
      />
      {showSidebarText && (
        <span className="font-bold text-sm ml-2">{link.label}</span>
      )}
...
const USER_MANAGEMENT_PREFIXES = ["/users"];

function isUserManagementPath(pathname: string) {
  return USER_MANAGEMENT_PREFIXES.some(
...
export function DashboardLayout({
  ...
  const [isBottomSectionCollapsed, setIsBottomSectionCollapsed] =
    useState(true);
  ...
  const activeLink = React.useMemo(() => {
    ...
    if (user?.role === Role.GUARDIAN && pathname === "/guardian") {
      ...
      const guardianMatch = allLinks.find((link) => {
        ...
        const linkView =
          new URLSearchParams(link.href.split("?")[1] || "").get("view") ||
          "overview";
    ...
    const pathOnlyMatch = allLinks.find(
      (l) => getHrefPath(l.href) === pathname && !l.href.includes("?"),
    );
    ...
    return allLinks
      .filter(
        (l) =>
          !l.href.endsWith("/dashboard") &&
          !l.href.endsWith("/admin") &&
          !l.href.includes("?"),
      )
  ...
  const getLinkHref = React.useCallback(
    (href: string) => {
      if (user?.role !== Role.GUARDIAN || !href.startsWith("/guardian"))
        return href;
  ...
  return (
    <div className="flex w-full bg-theme-bg h-full overflow-hidden relative select-none">
      ...
      {isMobileOpen && (
        <div
          className="fixed inset-0 bg-(--app-surface-overlay) backdrop-blur-sm z-80 lg:hidden transition-opacity duration-300"
      ...
      <aside
        className={`
                    ...
                    flex flex-col border-r border-border/70 bg-(--sidebar-bg) text-(--sidebar-text) shadow-[4px_0_24px_var(--shadow-color)]
      ...
      >
        ...
        <div
          className={`h-16 mt-14 lg:mt-0 flex items-center px-4 border-b border-border/70 shrink-0 ${isSidebarCompact ? "justify-center" : "justify-between"} gap-2 overflow-hidden relative group`}
        >
            ...
            <div className="ml-auto opacity-40 hover:opacity-100 transition-opacity">
              <BackButton
                {...(showSidebarText
                  ...
                  : { label: "" })}
                className="bg-transparent! border-none! rounded-md! shadow-none! text-(--sidebar-text)! py-1.5! px-3.5! outline-none! focus:outline-none!"
              />
            </div>
          </div>
        </div>
        ...
        <div
          className={`px-3 pb-[env(safe-area-inset-bottom,0px)] border-t border-border/70 shrink-0 relative ${isBottomSectionCollapsed ? "" : "bg-(--sidebar-panel)"}`}
        >
          {/* Toggle button sitting on top of border */}
          <button
            type="button"
            onClick={() =>
              setIsBottomSectionCollapsed(!isBottomSectionCollapsed)
            }
            className="absolute -top-3 left-1/2 flex h-6 min-w-12 -translate-x-1/2 items-center justify-center rounded-full border border-border/70 bg-(--sidebar-bg) px-4 text-(--sidebar-subtle) shadow-sm transition-colors hover:border-primary/40 hover:bg-(--sidebar-hover) hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/35"
          ...
          <div
            className={`overflow-hidden transition-all duration-300 ease-in-out ${isBottomSectionCollapsed ? "max-h-0 opacity-0" : "max-h-96 opacity-100"}`}
          >
            <div className="space-y-1.5 pb-3 pt-5">
                ...
                user?.role !== Role.PLATFORM_ADMIN && (
                  <>
                    ...
                    >
                      ...
                      <Mail
                        className={`w-4 h-4 shrink-0 ${pathname.includes("/mail") ? "text-primary-foreground" : "text-primary/80"}`}
                      />
                      {showSidebarText && (
                        <span className="ml-2 font-bold text-xs">Mail</span>
                      )}
                      {/* Mail Count */}
                      {mailCount.unread > 0 && (
                        <span
                          className={`ml-auto ${isSidebarCompact ? "absolute top-0 -right-0.5" : ""}`}
                        >
                    ...
                    {user?.role !== Role.STUDENT &&
                      user?.role !== Role.GUARDIAN && (
                        ...
                        >
                          ...
                          <MessageCircleQuestionMark
                            className={`w-4 h-4 shrink-0 ${pathname === "/contact" ? "text-primary-foreground" : "text-primary/80"}`}
                          />
                          {showSidebarText && (
              ...
              >
                ...
                <Key
                  className={`w-4 h-4 shrink-0 ${pathname.includes("/change-password") ? "text-primary-foreground" : "text-primary/80"}`}
                />
                {showSidebarText && (
                  <span className="ml-2 font-bold text-xs">
                    Change Password
              ...
              >
                ...
                {showSidebarText && (
                  <span className="ml-2 font-bold text-xs">Log out</span>
                )}
              </button>
            </div>
          </div>
          ...
          {user && (
            <div
              className={`mb-4 mt-2 flex min-h-13 items-center rounded-lg border border-border/70 bg-(--sidebar-panel) p-2 shadow-xs cursor-pointer transition-colors hover:border-primary/30 hover:bg-(--sidebar-hover) ${isSidebarCompact ? "lg:justify-center" : "gap-3"}`}
              onClick={() =>
                setIsBottomSectionCollapsed(!isBottomSectionCollapsed)
              }
            ...
            >
              <div className="relative flex h-9 w-9 shrink-0 items-center justify-center">
                <BrandIcon
                  ...
                  imageLoading="eager"
                />
                <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-(--sidebar-panel) bg-success" />
              </div>
              <div
                className={`min-w-0 overflow-hidden transition-all ${!showSidebarText ? "lg:hidden lg:w-0" : "w-auto flex-1"}`}
              >
                <div className="truncate text-sm font-black leading-tight text-(--sidebar-text)">
                  {user.name || user.email}
                </div>
                <div className="mt-0.5 truncate text-[11px] font-semibold leading-tight text-(--sidebar-subtle)">
