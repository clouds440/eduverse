...
export function getAttachmentPreviewKind(
  ...
  if (/\.(jpe?g|png|gif|webp|bmp|tiff|svg)$/i.test(lowerName)) return "image";
...
export function AttachmentPreviewCard({
  ...
  }, [href, initialFileSize, token]);

  const handleDownload = async () => {
    if (isDownloading) return;
    setIsDownloading(true);
    try {
      await downloadFile(href, downloadFileName, token);
  ...
  if (compact) {
    return (
      <div
        className={`doc-preview-card no-underline w-full max-w-full min-w-0 ${align === "right" ? "ml-auto" : ""}`}
      >
        {isImage ? (
          // --- Image preview (compact) ---
          <div className="flex flex-col items-center gap-1 p-1 mt-0.5 mx-1.5 rounded-xl bg-card/90 border border-border/50 transition-all duration-300 min-w-0">
          ...
          <div className="flex items-center gap-2 p-1.5 mt-0.5 mx-1.5 rounded-xl bg-card/90 border border-border/50 transition-all duration-300 min-w-0">
            <div
              className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg overflow-hidden shadow-sm"
  ...
  return (
    <div
      className={`doc-preview-card group no-underline w-full max-w-full min-w-0 ${align === "right" ? "ml-auto" : ""}`}
    >
      {isImage ? (
        // --- Image preview (full) ---
        <div className="flex flex-col p-0 rounded-2xl bg-card/95 border-2 border-border/70 mx-1 hover:border-primary/30 transition-all duration-300 backdrop-blur-sm overflow-hidden">
        ...
        <div className="flex items-center gap-2 p-2.5 sm:p-3 rounded-2xl bg-card/95 border-2 border-border/70 mx-1 hover:border-primary/30 transition-all duration-300 backdrop-blur-sm min-w-0">
          <div
            className="shrink-0 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-xl overflow-hidden transition-transform duration-300 group-hover:scale-110 shadow-sm ring-1 ring-white/10"
            style={{ background: fileInfo.bg, color: fileInfo.color }}
          >
            {isImage && !imageFailed ? (
              <img
