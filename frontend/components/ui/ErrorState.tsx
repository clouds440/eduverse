...
function getFriendlyError(
  ...
  if (
    ...
    return {
      ...
      message:
        "This screen asked for something the server could not find. Refresh and try once more.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      message:
        "The server hit a snag while loading this. Please try again in a moment.",
    };
  }
  ...
  if (connectionError && reachability === "checking") {
    return {
      ...
      message:
        "Checking whether this is your connection or the eduverse server.",
    };
  }
  ...
  if (
    ...
    (reachability === "offline" ||
      (isBrowserReportingOffline() && reachability !== "online"))
  ) {
    return {
      icon: WifiOff,
  ...
  if (connectionError) {
    return {
      ...
      message:
        "The eduverse server seems to be taking a quick nap. Please try again in a moment.",
    };
  }
...
export function ErrorState({
  ...
  const status = getErrorStatus(error);
  const shouldCheckReachability =
    isBrowserReportingOffline() || isConnectionError(error);
  ...
  const reachability: Reachability = shouldCheckReachability
    ? reachabilityResult?.key === reachabilityKey
  ...
  useEffect(() => {
    ...
    checkAppReachability(controller.signal)
      .then(() =>
        setReachabilityResult({ key: reachabilityKey, value: "online" }),
      )
      .catch(() =>
        setReachabilityResult({ key: reachabilityKey, value: "offline" }),
      )
