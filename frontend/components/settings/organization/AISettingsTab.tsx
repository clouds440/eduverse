...
export function AISettingsTab({
    ...
    const hasActiveSubscription =
        aiSettings &&
        activeAIPlan !== AISubscriptionPlan.NONE &&
        aiSettings.subscription.status === AISubscriptionStatus.ACTIVE;

    if (aiLoading && !aiSettings) {
        return (
                            ...
                            {AI_ACCESS_ROLE_ENTRIES.map(([role, field]) => {
                                ...
                                return (
                                    <div key={role} className="flex items-center justify-between gap-3 rounded-md border border-border/70 bg-background/60 px-3 py-2.5">
                                        <span className="text-sm font-semibold text-foreground">{AI_ROLE_LABELS[role] ?? role}</span>
                                        <Toggle
                                            checked={checked}
