...
import {
  blockDirectMessages,
  getDmBlockAction,
  unblockDirectMessages,
} from "@/lib/communicationBlocks";
...
export function DirectMessageBlockMenuItem({
  ...
  const action = useMemo(
    () => getDmBlockAction(chat, currentUser),
    [chat, currentUser],
  );
  ...
  const performAction = async () => {
    try {
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: {
          message: action.isBlockedByMe
            ? "Direct messages with this user unblocked"
            : "Direct messages with this user blocked",
          type: "success",
        },
      });
    } catch (error) {
      ...
      dispatch({
        ...
        payload: {
          message: action.isBlockedByMe
            ? "Failed to unblock direct messages"
            : "Failed to block direct messages",
          type: "error",
        },
      });
    }
