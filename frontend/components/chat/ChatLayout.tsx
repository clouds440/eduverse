...
function shouldRefreshCachedMessages(
  cachedMessages: ChatMessageWithMeta[],
  chat?: Chat,
) {
  ...
  const previewUpdatedAt = new Date(
    previewMessage.updatedAt || previewMessage.createdAt,
  ).getTime();
  const cachedUpdatedAt = new Date(
    latestCachedMessage.updatedAt || latestCachedMessage.createdAt,
  ).getTime();
...
function isChatDebugEnabled() {
  return (
    ...
    window.sessionStorage.getItem("eduverseChatDebug") === "1"
  );
}
  ...
  return chats
    .map((chat) => {
      ...
      if (!message?.encryptedContent)
        return `${chat.id}:plain:${message?.id || "none"}`;
      return [
        chat.id,
        message.id,
...
export function ChatLayout() {
  ...
  const { subscribe, joinRoom, leaveRoom, emit, isConnected } = useSocket({
    token,
    userId: user?.id,
    enabled: !!token,
  });
  ...
  const [chatSyncProgress, setChatSyncProgress] = useState<ChatSyncProgress>({
    status: "idle",
    done: 0,
    total: 0,
  });
  ...
  const [chatComposerStates, setChatComposerStates] =
    useState<ChatComposerStateMap>(() => getCachedComposerStates());
  const [chatEncryptionSendGates, setChatEncryptionSendGates] = useState<
    Record<string, ChatEncryptionSendGate>
  >({});
  const [isComposerStateHydrated, setIsComposerStateHydrated] = useState(false);
  const [isUploading, setIsUploading] = useState(false);
  const [isNewChatModalOpen, setIsNewChatModalOpen] = useState(false);
  const [isBlockedDmUsersModalOpen, setIsBlockedDmUsersModalOpen] =
    useState(false);
  ...
  const showUntrustedDeviceDialog = useCallback(
    (reason?: string) => {
      setConfirmConfig({
        isOpen: true,
        ...
        description:
          reason ||
          "This browser is not approved for secure Chat yet. Send an approval notification to a browser you already trust, then approve this browser from there.",
        cancelText: "OK",
        ...
        onConfirm: async () => {
          ...
          try {
            dispatchRef.current({
              ...
              payload: "e2ee-chat-device-approval",
            });
            const response = await requestCurrentDeviceTrust(token, {
            ...
            dispatchRef.current({
              type: "TOAST_ADD",
              payload: {
                message: trustedDeviceSetupErrorMessage(error),
                type: "error",
              },
            });
          } finally {
  ...
  const showRecipientNeedsTrustedDeviceDialog = useCallback(
    (description?: string) => {
      setConfirmConfig({
        isOpen: true,
        ...
        description:
          description ||
          "This message cannot be sent yet because at least one recipient has not trusted any browser. Ask them to sign in and approve a browser, then try again.",
        cancelText: "OK",
        hideConfirm: true,
        onConfirm: () => {},
      });
  ...
  const closeActiveChat = useCallback(
    (options?: { fromHistory?: boolean }) => {
      if (
        !options?.fromHistory &&
        !isDesktop &&
        isInPageBackSentinelState(window.history.state)
      ) {
        window.history.back();
        return;
      }
  ...
  const updateMessagesWithReadStatus = useCallback(
    (
      messages: ChatMessage[],
      readData: { messageId: string; userId: string },
    ): ChatMessage[] => {
      const readMessage = messages.find(
        (candidate) => candidate.id === readData.messageId,
      );
      const readAt = readMessage
        ? new Date(readMessage.createdAt).getTime()
        : null;
      return messages.map((m) => {
        const messageAt = new Date(m.createdAt).getTime();
        if (
          m.senderId !== readData.userId &&
          readAt !== null &&
          readAt >= messageAt
        ) {
          const readBy = Array.from(
            new Set([...(m.readBy || []), readData.userId]),
          );
          return { ...m, readBy };
        }
        return m;
    ...
    async (messageId: string) => {
      ...
      if (element) {
        ...
        try {
          ...
          dispatchRef.current({
            type: "TOAST_ADD",
            payload: { message: "Message not found in history", type: "error" },
          });
        } finally {
  ...
  const handleScroll = useCallback(() => {
    ...
    scrollFrameRef.current = window.requestAnimationFrame(() => {
      ...
      setShowScrollToBottom((prev) =>
        prev === shouldShowScrollToBottom ? prev : shouldShowScrollToBottom,
      );
  ...
  const readOnlyBannerRefCallback = useCallback(
  ...
  const fetchChats = useCallback(async () => {
    ...
    try {
      ...
      dispatchRef.current({
        type: "TOAST_ADD",
        payload: { message: "Failed to load chats", type: "error" },
      });
    }
  }, [token]);
  ...
  const applyDecryptedChatPreview = useCallback(
    (chatId: string, messageId: string, plaintext: string) => {
      setChats((prev) =>
        prev.map((chat) => {
          ...
          if (
            chat.id !== chatId ||
            !previewWithMeta ||
            previewWithMeta.id !== messageId ||
            previewWithMeta.decryptedContent === plaintext
          ) {
            return chat;
          }
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const cached = await Promise.all(
        previewMessages.map(async ({ chatId, message }) => ({
          chatId,
          message,
      ...
      for (const { chatId, message } of previewMessages) {
        ...
        try {
          ...
          console.warn(
            "Chat preview could not be opened on this browser",
            error,
          );
        } finally {
          done += 1;
  ...
  const fetchInitialMessages = useCallback(
    async (chatId: string, targetMsgId?: string | null) => {
      if (!token) return;
      ...
      try {
        ...
        if (targetMsgId) {
          res = await api.chat.getChatMessages(chatId, token, {
            limit: 35,
            aroundId: targetMsgId,
          });
        } else {
          res = await api.chat.getChatMessages(chatId, token, {
            limit: 35,
        ...
        if (
          !mountedRef.current ||
          fetchId !== fetchSequenceRef.current ||
          chatId !== activeChatIdRef.current
        ) {
          chatDebug("messages:fetch-stale", {
            chatId,
            fetchId,
            currentFetchId: fetchSequenceRef.current,
        ...
        chatDebug("messages:set-initial", {
          chatId,
          fetchId,
        ...
        setHasMoreMessages(
          res.hasMoreBefore ?? res.currentPage < res.totalPages,
        );
        setHasMoreAfter(res.hasMoreAfter ?? false);
        setIsViewingHistory(res.hasMoreAfter ?? false);
  ...
  useEffect(() => {
    ...
    if (
      !targetMessageId &&
      !shouldRefreshCachedMessages(cachedMessages, activeChatPreview)
    ) {
      chatDebug("messages:set-cached", {
        activeChatId,
  ...
  useEffect(() => {
    ...
    const unsubscribePresenceUpdate = subscribe(
      "presence:update",
      (payload: unknown) => {
        const event = payload as PresenceUpdateEvent;
        setOnlineUsers((prev) =>
          updateOnlineUsersFromPresenceEvent(prev, event),
        );
        if (!event.isOnline) {
          setTypingByChatId((prev) =>
            removeTypingUserFromAllChats(prev, event.userId),
          );
        }
  ...
  useEffect(() => {
    ...
    const unsubscribePresenceState = subscribe(
      "presence:state",
      (payload: unknown) => {
        const event = payload as PresenceStateEvent;
        setOnlineUsers((prev) =>
          updateOnlineUsersFromPresenceState(prev, event),
        );
      },
    );
  ...
  useEffect(() => {
    return () => {
      ...
      if (typingChatIdRef.current && emit) {
        emit("chat:typing", {
          chatId: typingChatIdRef.current,
          isTyping: false,
        });
  ...
  const loadEarlierMessages = async () => {
    ...
    try {
      ...
      if (isViewingHistory && messages.length > 0) {
        ...
        if (!mountedRef.current || activeChatIdRef.current !== requestedChatId)
          return;
        chatDebug("messages:set-earlier-history", {
        ...
        if (!mountedRef.current || activeChatIdRef.current !== requestedChatId)
          return;
        chatDebug("messages:set-earlier-page", {
          chatId: requestedChatId,
          count: res.data.length,
          page: nextPage,
        });
  ...
  useEffect(() => {
    ...
    const unsubMessage = subscribe("chat:message", (newMsg: unknown) => {
      ...
      const updatedCached = reconcileIncomingMessage(
        cachedMessages,
        message,
        user.id,
        null,
      ...
      if (message.chatId === activeChatId) {
        setMessages((prev) => {
          const pendingId =
            message.senderId === user.id ? pendingMessageIdRef.current : null;
        ...
        if (isAtBottom || message.senderId === user.id) {
          setTimeout(
            () =>
              scrollToBottom(
                message.senderId === user.id ? "instant" : "smooth",
              ),
            50,
          );
        } else {
      ...
      setChats((prevChats) => {
        ...
        if (chatIndex > -1) {
          const previousPreview = prevChats[chatIndex].messages?.[0] as
            | ChatMessageWithMeta
            | undefined;
          const previewMessage =
            previousPreview?.id === message.id &&
            previousPreview.decryptedContent
              ? {
                  ...message,
                  decryptedContent: previousPreview.decryptedContent,
                }
              : message;
          const updatedChat = {
            ...prevChats[chatIndex],
            ...
            unreadCount:
              message.chatId !== activeChatId && message.senderId !== user.id
    ...
    const unsubCommunicationBlockUpdate = subscribe(
      "chat:communication-block:update",
      () => {
        refreshChats();
      },
    );
    ...
    const unsubRoomLeft = subscribe("roomLeft", (data: unknown) => {
      ...
      if (leftData.forced && leftData.roomId === `chat:${activeChatId}`) {
        ...
        setChats((prev) =>
          prev.map((c) =>
            c.id === activeChatId ? { ...c, isActive: false } : c,
          ),
        );
      }
    });
  ...
  const { data: mentionOptions, isLoading: isLoadingMentionOptions } =
    useChatMentionOptions(
      activeChatId,
      token,
  ...
  const {
    ...
    editingMessage,
    mentionTargets,
  } = activeChatComposerState;
  const [stagedFilePreviewUrls, setStagedFilePreviewUrls] = useState<
    Array<string | null>
  >([]);
  ...
  const baseCanSendMessage = useMemo(() => {
    ...
    if (
      ...
      activeChat.directMessageBlock?.isBlocked
    )
      return false;
    ...
    return (
      currentUserParticipant.role === ChatParticipantRole.ADMIN ||
      currentUserParticipant.role === ChatParticipantRole.MOD
    );
  ...
  const effectiveChatEncryptionGate: ChatEncryptionSendGate | undefined =
    useMemo(() => {
      if (!activeChat || !baseCanSendMessage) return undefined;
      return (
        activeChatEncryptionGate || {
  ...
  const encryptionSendBlockLabel =
    effectiveChatEncryptionGate?.status === "blocked"
      ? effectiveChatEncryptionGate.message
      : null;
  const canSendMessage =
    baseCanSendMessage && effectiveChatEncryptionGate?.status !== "blocked";

  useEffect(() => {
    const participantUserIds = activeParticipantKey
      ? activeParticipantKey.split("|")
      : [];
    if (
      !token ||
      !user?.id ||
    ...
    const run = async () => {
      try {
        ...
        const missingUserIds = getUserIdsWithoutTrustedRecipientDevices(
          recipientDevices,
          participantUserIds,
        );
        const missingRecipientIds = missingUserIds.filter(
        ...
        if (
          activeChatType === ChatType.DIRECT &&
          missingRecipientIds.length > 0
        ) {
          const message =
            "The recipient needs to trust a browser before messages can be sent.";
          const description =
            "This chat cannot accept new secure messages yet because the recipient has not trusted any browser. Ask them to sign in and approve a browser, then come back to this chat.";
  ...
  const handleDeleteChat = async (chatId: string) => {
    ...
    try {
      ...
      dispatchRef.current({
        type: "TOAST_ADD",
        payload: { message: "Chat removed from view", type: "success" },
      });
    } catch (err) {
      console.error("Failed to delete chat:", err);
      dispatchRef.current({
        type: "TOAST_ADD",
        payload: { message: "Failed to delete chat", type: "error" },
      });
    }
  };
  ...
  const performClearChatHistory = async (chatId: string) => {
    ...
    try {
      ...
      dispatchRef.current({
        type: "TOAST_ADD",
        payload: { message: "Chat history cleared", type: "success" },
      });
    } catch (err) {
      console.error("Failed to clear chat history:", err);
      dispatchRef.current({
        type: "TOAST_ADD",
        payload: { message: "Failed to clear chat history", type: "error" },
      });
    }
  };
  ...
  const filteredChats = useMemo(() => {
    ...
    result = result.filter((chat) => {
      ...
      const hiddenAt =
        myParticipant?.hiddenAt ||
        (chat as Chat & { hiddenAt?: string | Date | null }).hiddenAt;
  ...
  const activeChatParticipantIds = useMemo(
    () => activeParticipants.map((p) => p.userId),
    [activeParticipants],
  );

  const updateComposerStateForChat = useCallback(
    (
      chatId: string | null,
      patch: Parameters<typeof updateChatComposerState>[2],
    ) => {
      setChatComposerStates((prev) =>
        updateChatComposerState(prev, chatId, patch),
      );
    },
    [],
  );

  const updateActiveComposerState = useCallback(
    (patch: Parameters<typeof updateChatComposerState>[2]) => {
  ...
  const handleSelectMentionTarget = useCallback(
    (target: Parameters<typeof insertMentionToken>[0]["target"]) => {
      if (!mentionTrigger) return;
      const selectionStart =
        textareaRef.current?.selectionStart ||
        mentionTrigger.start + mentionSearchQuery.length + 1;
  ...
  const handleSendMessage = useCallback(
    async (retryMessage?: ChatMessageWithMeta) => {
      const chatId = activeChatId;
      const draftText =
        retryMessage?.retryPayload?.draftText ?? messageDraft.trim();
      const filesToSend =
        retryMessage?.retryPayload?.stagedFiles ?? stagedFiles;
      const replyTarget =
        retryMessage?.retryPayload?.replyToMessage ?? replyToMessage;
      const targetsForSend =
        retryMessage?.retryPayload?.mentionTargets ?? mentionTargets;
      ...
      if (
        ...
        !chatId ||
        !activeChat ||
        (!draftText && filesToSend.length === 0)
      )
        return;
      if (
        activeChat?.type === ChatType.DIRECT &&
        ...
        dispatchRef.current({
          type: "TOAST_ADD",
          payload: {
            message: "Direct messages are blocked in this chat",
            type: "info",
          },
        });
        return;
      ...
      try {
        ...
        if (!editingMessage) {
          if (!tempMessageId) {
            ...
            const optimisticAttachments =
              buildOptimisticAttachmentMarkdown(filesToSend);

            const optimisticContent = (
              optimisticAttachments + (draftText ? `\n${draftText}` : "")
            ).trim();
        ...
        if (editingMessage) {
          ...
          void cacheDecryptedChatMessageContent(
            sentMessage,
            token,
            sentMessageWithPlaintext.decryptedContent,
          );
        ...
        setTimeout(() => {
          const ta =
            textareaRef.current as unknown as HTMLTextAreaElement | null;
          if (ta) {
        ...
        setIsUploading(false);
        if (
          err instanceof E2EEError &&
          err.code === "RECIPIENT_NO_TRUSTED_DEVICE"
        ) {
          showRecipientNeedsTrustedDeviceDialog();
        } else if (
          err instanceof E2EEError &&
          err.code === "NO_TRUSTED_DEVICE"
        ) {
          showUntrustedDeviceDialog();
        } else {
          dispatchRef.current({
  ...
  const handleDeleteMessage = useCallback(
    (messageId: string) => {
      ...
      const failedMessage = messages.find(
        (m) => m.id === messageId && m.clientStatus === "failed",
      );
        ...
        onConfirm: async () => {
          if (failedMessage) {
            ...
            try {
              ...
              dispatchRef.current({
                type: "TOAST_ADD",
                payload: { message: "Failed to delete message", type: "error" },
              });
            }
          }
        },
  ...
  const handleRemoveParticipant = (participantUserId: string) => {
    ...
    const participantName =
      activeChat?.participants?.find((p) => p.userId === participantUserId)
        ?.user?.name || "this participant";
    ...
    setConfirmConfig({
      ...
      onConfirm: async () => {
        try {
          ...
          dispatchRef.current({
            type: "TOAST_ADD",
            payload: { message: "Participant removed", type: "success" },
          });
          fetchChats();
        } catch (err) {
          console.error(err);
          dispatchRef.current({
            type: "TOAST_ADD",
            payload: { message: "Failed to remove participant", type: "error" },
          });
        }
      },
    });
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length < files.length) {
      dispatchRef.current({
        ...
        payload: {
          message:
            "Some attachments were skipped because the file type is not allowed.",
          type: "info",
        },
      });
    }
    ...
    if (stagedFiles.length + validFiles.length > 5) {
      dispatchRef.current({
        type: "TOAST_ADD",
        payload: { message: "Maximum 5 attachments allowed", type: "info" },
      });
      return;
    }
  ...
  const handleEditMessage = useCallback(
    (msg: ChatMessageWithMeta) => {
      if (msg.clientStatus === "failed") return;
      const editableContent = isEncryptedChatMessage(msg)
      ...
      if (!editableContent) {
        dispatchRef.current({
          type: "TOAST_ADD",
          payload: {
            message: "This message is not available to edit on this device",
            type: "info",
          },
        });
        return;
  ...
  const handleCopyText = useCallback((msg: ChatMessageWithMeta) => {
    const copyContent = isEncryptedChatMessage(msg)
      ? msg.decryptedContent
      : msg.content;
    if (!copyContent) {
      dispatchRef.current({
        type: "TOAST_ADD",
        payload: {
          message: "This message is not available to copy on this device",
          type: "info",
        },
      });
      return;
    ...
    navigator.clipboard
      ...
      .catch((err) => {
        ...
        dispatchRef.current({
          type: "TOAST_ADD",
          payload: { message: "Failed to copy text", type: "error" },
        });
      });
  }, []);

  const handleMessageDecrypted = useCallback(
  ...
  useEffect(() => {
    ...
    const encryptedMessages = messages.filter((message) =>
      isEncryptedChatMessage(message),
    );
    ...
    const run = async () => {
      ...
      const cachedById = new Map(
        cached
          .filter(
            (
              entry,
            ): entry is { message: ChatMessageWithMeta; plaintext: string } =>
              Boolean(entry.plaintext),
          )
      ...
      for (const message of encryptedMessages) {
        ...
        try {
          ...
          console.warn(
            "Chat message could not be opened on this browser",
            error,
          );
        }
  ...
  const openMessageContextMenu = useCallback(
    (targetMsg: ChatMessageWithMeta, x: number, y: number) => {
  ...
  const retrySendMessage = useCallback(
    (targetMsg: ChatMessageWithMeta) => {
      void handleSendMessage(targetMsg);
    },
    [handleSendMessage],
  );

  const scrollToMessageFromRow = useCallback(
    (messageId: string) => {
  ...
  const isComposerExpanded =
    messageDraft.length > (isDesktop ? 150 : 30) || messageDraft.includes("\n");
  const chatRecipient = activeChat?.participants?.find(
    (p) => p.userId !== user.id,
  );
  const activeChatName =
    activeChat?.type === ChatType.GROUP
      ? activeChat.name
      : chatRecipient?.user?.role === Role.SUPER_ADMIN
        ? "Super Admin"
        : chatRecipient?.user?.name || "Unknown User";
  return (
    <div className="flex h-full min-w-0 bg-background lg:shadow-xl lg:shadow-foreground/5 lg:border border-border overflow-hidden relative">
      ...
      <div
        ...
        `}
      >
        <div className="px-4 sm:px-5 py-2 border-b border-border/60 bg-background/80 flex justify-between items-center">
          <div>
            <h2 className="text-[17px] sm:text-lg font-black text-foreground tracking-tight leading-tight">
              Messages
            </h2>
            <p className="text-[11px] sm:text-[12px] text-muted-foreground font-semibold tracking-wide mt-1">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {isLoadingChats ? (
            ...
            filteredChats.map((chat) => {
              ...
              const displayName =
                otherUsers[0]?.user?.role === Role.SUPER_ADMIN
                  ? "Super Admin"
              ...
              const isCleared =
                ...
                new Date(lastMsgRaw.createdAt) <=
                  new Date(myParticipant.clearedAt);
              ...
              return (
                <div
                  key={chat.id}
                  className="relative group mx-2 mb-1 first:mt-1"
                >
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    {...getLongPressHandlers(
                      ...
                      touchChatTimerRef,
                      touchChatStartPosRef,
                      touchChatHasTriggeredRef,
                    )}
                  ...
                  >
                      ...
                      <ChatAvatar
                        ...
                        isOnline={
                          !!(
                            ...
                            onlineUsers[otherUsers[0].user.id]
                          )
                        }
                      />
                      {hasUnread && (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1">
                        <h4
                          className={`text-[13px] font-black tracking-tight truncate pr-2 leading-tight ${isActive ? "text-primary" : hasUnread ? "text-foreground" : "text-foreground/90"}`}
                        >
                          {displayName}
                        </h4>
                        {lastMsg && (
                          <span
                            className={`text-[10px] font-bold shrink-0 ml-2 uppercase tracking-tighter ${hasUnread ? "text-primary" : "text-muted-foreground/40"}`}
                          >
                      ...
                      <div className="flex items-center justify-between">
                        <p
                          className={`text-[11px] sm:text-[13px] truncate flex-1 ${hasUnread ? "text-foreground font-medium" : "text-muted-foreground"}`}
                        >
                          {(() => {
                            const chatTypingUsers = getTypingUsersForChat(
                              typingByChatId,
                              chat.id,
                              user?.id,
                            );
                  ...
                  {chatMenuOpenId === chat.id && (
                    <div
                      ...
                      className="absolute right-2 top-full mt-1 bg-card border border-border rounded-xl shadow-xl py-1 z-50 min-w-40 overflow-hidden"
                    >
                      <button
                        type="button"
                        onClick={(e) => {
      ...
      >
        {activeChat ? (
          <>
            {/* Chat Header */}
            <div className="relative w-full px-3 sm:px-4 py-1.5 border-b border-border/60 flex items-center justify-between z-20 bg-background/95 shadow-sm shadow-foreground/5">
              <button
                type="button"
              ...
              >
                  ...
                  targetUser={
                    activeChat.type === ChatType.GROUP
                      ? {
                          ...
                          avatarUpdatedAt: activeChat.avatarUpdatedAt,
                        }
                      : chatRecipient?.user
                  }
                  className="w-9 h-9 sm:w-10 sm:h-10 transition-transform group-hover/header:scale-105"
                  isOnline={
                    !!(directChatTarget?.id && onlineUsers[directChatTarget.id])
                  }
                  imageLoading="eager"
                />
                <div className="flex min-w-0 flex-col items-start">
                  ...
                  {typingIndicatorLabel ? (
                    ...
                    (() => {
                      const otherParticipant = chatRecipient;
                      ...
                      return (
                        <span className="flex items-center gap-2 text-[10px]">
                          <span className={isOnline ? "text-success" : ""}>
                            {status}
                          </span>
                          {status && (
                          ...
                          <span
                            className={`capitalize ${status && "hidden md:block"}`}
                          >
                            {role}
                          </span>
                        </span>
            ...
            <div className="flex-1 flex overflow-hidden relative bg-background/70">
              <div
                ...
                style={{
                  paddingBottom: !isDesktop
                    ...
                    : (canSendMessage ? composerHeight : readOnlyBannerHeight) +
                      16,
                }}
              >
                {isLoadingMessages ? (
                  ...
                  <>
                    ...
                    <ChatMessageList
                      ...
                      contextMessageId={contextMenu?.msg.id}
                      isDesktop={isDesktop}
                      userId={user?.id}
                      activeParticipantsOnline={activeParticipantsOnline}
                      retryDisabled={
                        sendLockRef.current || isSending || isUploading
                      }
              ...
              {isViewingHistory && (
                <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30 bg-foreground text-background px-4 py-2 rounded-full shadow-lg flex items-center space-x-3 text-[13px] font-semibold animate-in slide-in-from-top duration-300">
                  ...
                  <button
                    onClick={() =>
                      activeChatId && fetchInitialMessages(activeChatId)
                    }
              ...
              {((showScrollToBottom && !isLoadingMessages) ||
                isViewingHistory) && (
                <button
                  type="button"
                  onClick={() =>
                  ...
                  style={{
                    bottom: !isDesktop
                      ...
                      : (canSendMessage
                          ? composerHeight
                          : readOnlyBannerHeight) + 30,
                  }}
                  title={
                    isViewingHistory ? "Jump to Present" : "Scroll to bottom"
                  }
                >
                  <ArrowDown
                    size={18}
                    className="group-hover:translate-y-0.5 transition-transform text-primary/80"
                  />
              ...
              >
                <div className="px-4 py-3 border-b border-border/60 flex justify-between items-center bg-card/45">
                  <div>
                    <h4 className="font-black text-foreground text-[13px] sm:text-[14px]">
                      Members
                    </h4>
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
                  {activeParticipants.map((p) => (
                    <div
                      key={p.id}
                      className="flex items-center justify-between gap-2 rounded-2xl p-1.5 transition-colors hover:bg-card/80 group/item"
                    >
                      <div
                        onClick={() => {
                          if (p.user?.organizationId) {
                            setShowParticipants(false);
                        ...
                        title={
                          p.user?.organizationId
                            ? `View ${p.user?.name || "member"} profile`
                            : "Profile unavailable"
                        }
                      >
                        <ChatAvatar
                          ...
                          isOnline={!!onlineUsers[p.userId]}
                        />
                        <div className="min-w-0">
                          <p
                            className="text-[12px] sm:text-[13px] font-bold truncate"
                            style={{ color: getUserColor(p.user?.id) }}
                          >
                            {p.user?.name}{" "}
                            {p.userId === user.id && (
              ...
              {canSendMessage ? (
                ...
                >
                    ...
                    <div className="mb-1 px-4 py-2.5 bg-card/90 rounded-xl flex items-center justify-between animate-in slide-in-from-bottom-2 duration-300 shadow-sm ring-1 ring-primary/50">
                      <div className="flex-1 min-w-0 pr-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-primary mb-1">
                          {editingMessage
                            ? "Editing Message"
                            : `Replying to ${replyToMessage?.sender?.name == user.name ? "Yourself" : replyToMessage?.sender?.name || "Message"}`}
                        </p>
                        <div
                          onClick={() => {
                            if (replyToMessage)
                              scrollToMessage(replyToMessage.id);
                            else scrollToMessage(editingMessage!.id);
                          }}
                          className="text-[12px] text-muted-foreground truncate cursor-pointer italic"
                        >
                          <RichMessageRenderer
                            content={getTruncatedMessagePreview(
                  ...
                  {mentionTargets.length > 0 && !editingMessage && (
                    <div className="mb-1.5 sm:mb-1 px-2.5 sm:px-3 py-2 sm:py-2 mr-2 bg-muted border-l-4 border-primary rounded-lg flex items-center justify-between animate-in slide-in-from-bottom duration-200">
                      <div className="flex-1 min-w-0 pr-2 sm:pr-3">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {mentionTargets.map((target) => (
                            <span
                              key={`${target.type}:${target.userId || target.role || `${target.audienceRole}:${target.scopeType}:${target.scopeId}`}`}
                              className="text-[11px] sm:text-[12px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-md font-medium"
                            >
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          updateActiveComposerState({ mentionTargets: [] })
                        }
                    ...
                    <div className="flex flex-row gap-1 mb-1 mr-2 scrollbar-thin overflow-x-auto scroll-x-auto">
                      {stagedFiles.map((file, i) => {
                        ...
                        const isImage =
                          ...
                          stagedFilePreviewUrls[i];

                        return (
                          <div
                            key={i}
                            className="group relative flex items-center bg-card border border-border/50 p-2 rounded-xl hover:border-primary/50 hover:shadow-md transition-all duration-300 min-w-50 max-w-70"
                          >
                            {/* Preview Thumbnail */}
                            {isImage && stagedFilePreviewUrls[i] ? (
                              ...
                              <div
                                className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center mr-2.5 shadow-sm shrink-0"
                                style={{
                                  background: fileInfo.bg,
                                  color: fileInfo.color,
                                }}
                              >
                                {fileInfo.label === "ARCHIVE" ? (
                            ...
                            <div className="flex-1 min-w-0 mr-1">
                              <p className="text-[11px] sm:text-[12px] font-bold text-foreground truncate tracking-tight">
                                {file.name || "Attachment"}
                              </p>
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <span
                                  className="text-[9px] font-black uppercase px-1 py-0.5 rounded-sm text-white"
                  ...
                  {/* Composer Row */}
                  <div
                    className={`flex flex-row items-end mb-1.5 sm:mb-2 min-w-0 max-w-full`}
                  >
                    <input
                      title="Upload File"
                      type="file"
                    ...
                    <div className="flex-1 min-w-0 max-w-full bg-card/95 border-2 border-border/70 rounded-3xl focus-within:bg-background/70 focus-within:border-primary/30 focus-within:ring-4 focus-within:ring-primary/10 transition-all shadow-lg shadow-foreground/5">
                      {/* Top row */}
                      <div
                        className={`flex items-end min-w-0 transition-all duration-500 ease-out ${isComposerExpanded ? "px-3 pt-2" : "px-2"}`}
                      >
                          ...
                          <textarea
                            ...
                            onChange={(e) => {
                              ...
                              updateActiveComposerState({
                                messageDraft: newVal,
                                ...(wasMentionRemoved
                                  ? { mentionTargets: remainingMentions }
                                  : {}),
                              });
                            ...
                            onPaste={(e) => {
                              const rawFiles = extractFilesFromClipboard(
                                e.nativeEvent,
                              );
                    ...
                    <div
                      className={`flex items-end justify-center transition-all duration-300 ease-out ${
                ...
                >
                  <div className="flex min-h-13 items-center justify-center gap-2 rounded-2xl border border-border/70 bg-muted/70 px-4 py-3 text-muted-foreground shadow-sm opacity-80 cursor-not-allowed">
                    ...
                    <p className="text-center text-[11px] font-semibold sm:text-[12px] md:text-[13px]">
                      {directMessageBlockLabel ||
                        encryptionSendBlockLabel ||
          ...
          <div className="flex-1 flex flex-col items-center justify-center chat-bg-pattern px-4">
            <div className="flex flex-col items-center justify-center p-14 bg-card/60 rounded-xl shadow-lg">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground max-w-xs sm:max-w-sm text-center">
      ...
      <ConfirmDialog
        ...
        onClose={() =>
          setDeleteConfirmConfig({ isOpen: false, chatId: null, chatName: "" })
        }
        onConfirm={() => {
