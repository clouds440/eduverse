...
export default memo(function CourseMaterials({
  sectionId,
  ...
  const handleDelete = async () => {
    ...
    try {
      dispatch({
        ...
        payload: `material-delete-${target.id}`,
      });
      await api.courseMaterials.deleteMaterial(target.id, token);
      dispatch({
        type: "TOAST_ADD",
        payload: { message: "Material deleted successfully", type: "success" },
      });
      fetchMaterials();
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: { message: "Failed to delete material", type: "error" },
      });
      setDeletingMaterial(null);
    } finally {
      dispatch({
  ...
  const handleDownload = async (file: { path: string; filename: string }) => {
    try {
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: { message: "Failed to download file", type: "error" },
      });
      console.error(error);
    }
  };
    ...
    return (
      <div className="grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-3">
        {[...Array(3)].map((_, index) => (
          <div
            key={index}
            className="h-44 min-w-0 animate-pulse rounded-lg border border-border/70 bg-muted/35"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="min-w-0 max-w-full space-y-4 overflow-hidden">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="wrap-break-word text-xs font-semibold text-muted-foreground">
            Files, external links, and class resources shared with this section.
          </p>
        </div>
        {isTeacherAssigned && (
          <Button
      ...
      {materials.length === 0 ? (
        <div className="min-w-0 rounded-lg border border-dashed border-border/70 bg-background/60 px-4 py-10 text-center sm:px-6">
          ...
          <p className="mt-1 text-xs font-semibold text-muted-foreground">
            {isTeacherAssigned
              ? "Add a file or link when students need a resource."
              : "Materials shared by the teaching team will appear here."}
          </p>
        </div>
      ) : (
        <div className="grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
          {materials.map((material) => {
            ...
            return (
              <article
                key={material.id}
                className="min-w-0 max-w-full overflow-hidden rounded-lg border border-border/70 bg-card p-3 shadow-sm sm:p-4"
              >
                <div className="flex min-w-0 items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex min-w-0 flex-wrap items-center gap-2">
                      <Badge variant="primary" size="sm" icon={FileText}>
                        Material
                      </Badge>
                    ...
                    <h3 className="mt-3 line-clamp-2 wrap-break-word text-base font-black leading-tight text-foreground">
                      {material.title}
                    </h3>
                  </div>
      ...
      {viewingMaterial && (
        ...
        >
          <div className="space-y-6">
            <div>
              <h3 className="text-xl font-black text-card-text">
                {viewingMaterial.title}
              </h3>
              {viewingMaterial.section?.name && (
                <CourseSectionLabel
                  section={viewingMaterial.section}
                  as="p"
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-muted-foreground">
              <span>
                Added {new Date(viewingMaterial.createdAt).toLocaleDateString()}
              </span>
              {viewingMaterial.creator?.name && (
                <span>by {viewingMaterial.creator.name}</span>
              )}
            </div>
                ...
                <div className="space-y-2">
                  {viewingMaterial.files.map((file) => {
                    ...
                    return (
                      <div
                        key={file.id}
                        className="flex min-w-0 items-center gap-3 rounded-lg border border-border/60 bg-background/70 p-3"
                      >
                        <FileIcon className="h-5 w-5 shrink-0 text-primary" />
                        <span className="min-w-0 flex-1 truncate text-sm font-semibold text-card-text/80">
                          {file.filename}
                        </span>
      ...
      <ConfirmDialog
        ...
        loadingId={
          deletingMaterial
            ? `material-delete-${deletingMaterial.id}`
            : undefined
        }
      />
    </div>
...
function UploadMaterialModal({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      dispatch({
        type: "UI_START_PROCESSING",
        payload: material ? `material-edit-${material.id}` : "material-create",
      });
      if (material) {
      ...
      dispatch({
        type: "TOAST_ADD",
        payload: { message: "Failed to save material", type: "error" },
      });
    } finally {
      dispatch({
        type: "UI_STOP_PROCESSING",
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={material ? "Edit Course Material" : "Upload Course Material"}
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <ExternalLinkInput
          ...
          disabled={
            state.ui.processing["material-create"] ||
            Boolean(
              material && state.ui.processing[`material-edit-${material.id}`],
            )
          }
        />
        ...
        <div>
          ...
          <div className="rounded-lg border-2 border-dashed border-border p-6 text-center transition-colors hover:border-primary/50">
            <input
              ...
              multiple
              onChange={(event) =>
                event.target.files && handleFileUpload(event.target.files)
              }
              ...
              disabled={
                ...
                Boolean(
                  material &&
                  state.ui.processing[`material-edit-${material.id}`],
                )
              }
            />
            <label
            ...
            <div className="mt-4 space-y-2">
              ...
              {existingFiles.map((file) => {
                ...
                return (
                  <div
                    key={file.id}
                    className="flex min-w-0 items-center gap-3 rounded-lg border border-border/50 bg-muted/30 p-3"
                  >
                    <FileIcon className="h-5 w-5 shrink-0 text-primary" />
                    <span className="min-w-0 flex-1 truncate text-sm font-semibold text-card-text/80">
                      {file.filename}
              ...
              {pendingFiles.map((file, index) => {
                ...
                return (
                  <div
                    key={`${file.name}-${index}`}
                    className="flex min-w-0 items-center gap-3 rounded-lg border border-primary/20 bg-primary/5 p-3"
                  >
                    <FileIcon className="h-5 w-5 shrink-0 text-primary" />
                    <span className="min-w-0 flex-1 truncate text-sm font-semibold text-card-text/80">
                      {file.name}
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <Button
            type="submit"
            loadingId={
              material ? `material-edit-${material.id}` : "material-create"
            }
            disabled={!title}
          >
