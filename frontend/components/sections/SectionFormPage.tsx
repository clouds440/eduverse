...
import { sectionComponentTypeOptions } from '@/lib/sectionRelationships';
...
export function SectionFormPage({ sectionId }: { sectionId?: string }) {
    ...
    const [removedStudentsExcluded, setRemovedStudentsExcluded] = useState(false);
    const [removedStudentsReason, setRemovedStudentsReason] = useState('');
    ...
    const removedStudentIds = useMemo(() => (section?.students || []).map((student) => student.id).filter((id) => !form.studentIds.includes(id)), [form.studentIds, section?.students]);
        ...
        try {
            ...
            if (section) {
                ...
                await Promise.all(removed.map((studentId) => api.org.withdrawStudentFromSection(studentId, saved.id, token, {
                    wasExcluded: removedStudentsExcluded,
                    reason: removedStudentsReason || undefined,
                })));
            }
    ...
    return <div className="mx-auto w-full max-w-5xl overflow-y-auto px-4 py-8"><PageHeader title={isEdit ? 'Edit Section' : 'Create Section'} description="Configure one course delivery in an institute academic cycle." icon={Layers} breadcrumbs={[{ label: 'Sections', href: '/sections' }, { label: isEdit ? 'Edit' : 'Create' }]} className="mb-7" /><form onSubmit={submit} className="space-y-7">
        ...
        <section><Intro title="People" text="Assign teachers now. When editing, direct section enrollment can also be managed here; cohort-driven enrollment remains owned by its cohort offering." /><div className="space-y-5 rounded-lg border border-border/70 bg-card/75 p-5"><Field label="Teachers"><CustomMultiSelect values={form.teacherIds} onChange={(teacherIds) => setForm({ ...form, teacherIds })} options={(teachers?.data || []).map((item) => ({ value: item.id, label: item.user?.name || item.user?.email || 'Teacher' }))} /></Field>{affectedSchedules.length > 0 && <div className="space-y-4 rounded-md border border-warning/35 bg-warning/10 p-4"><div className="flex gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-warning" /><div><p className="text-sm font-black">Removed teachers own existing schedules</p><p className="text-xs text-muted-foreground">Resolve {affectedSchedules.length} affected schedule{affectedSchedules.length === 1 ? '' : 's'} before saving.</p></div></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Resolution"><CustomSelect value={scheduleResolution} onChange={setScheduleResolution} options={[{ value: '', label: 'Choose resolution' }, { value: 'MOVE', label: 'Move schedules' }, { value: 'DELETE', label: 'Delete schedules' }]} /></Field>{scheduleResolution === 'MOVE' && <Field label="Move to teacher"><CustomSelect searchable value={scheduleTeacherId} onChange={setScheduleTeacherId} options={remainingTeacherOptions} placeholder="Choose remaining teacher" /></Field>}</div></div>}{isEdit && <Field label="Directly enrolled students"><CustomMultiSelect values={form.studentIds} onChange={(studentIds) => setForm({ ...form, studentIds })} options={(students?.data || []).map((item) => ({ value: item.id, label: item.user?.name || item.user?.email || 'Student' }))} /></Field>}{removedStudentIds.length > 0 && <div className="space-y-4 rounded-md border border-warning/35 bg-warning/10 p-4"><div className="flex gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-warning" /><div><p className="text-sm font-black">Remove-only preserves records</p><p className="text-xs font-semibold text-muted-foreground">You are removing {removedStudentIds.length} student{removedStudentIds.length === 1 ? '' : 's'} from this section. Attendance, grades, submissions, and history remain preserved. If this is a move to another section, use Academic Transitions &gt; Reassign Students instead.</p></div></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Transcript treatment"><CustomSelect value={removedStudentsExcluded ? 'EXCLUDE' : 'INCLUDE'} onChange={(value) => setRemovedStudentsExcluded(value === 'EXCLUDE')} options={[{ value: 'INCLUDE', label: 'Preserve and include in transcript merit' }, { value: 'EXCLUDE', label: 'Preserve but exclude from GPA and merit' }]} /></Field><Field label="Reason"><Input value={removedStudentsReason} onChange={(event) => setRemovedStudentsReason(event.target.value)} placeholder="Optional removal reason" /></Field></div></div>}</div></section>
