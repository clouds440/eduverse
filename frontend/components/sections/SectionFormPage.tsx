...
import { AcademicCycle, Cohort, Course, PaginatedResponse, ProgramDeliveryOption, Role, Room, Section, Student, Teacher } from '@/types';
...
export function SectionFormPage({ sectionId }: { sectionId?: string }) {
    ...
    const [form, setForm] = useState({ name: '', code: '', courseId: '', academicCycleId: '', defaultRoomId: '', color: DEFAULT_SECTION_COLOR, programStageOfferingId: '', requirementIds: [] as string[], cohortOfferingId: '', teacherIds: [] as string[], studentIds: [] as string[] });
    ...
    useEffect(() => {
        ...
        setForm({ name: section.name, code: section.code, courseId: section.courseId || '', academicCycleId: section.academicCycleId || '', defaultRoomId: section.defaultRoomId || '', color: section.color || DEFAULT_SECTION_COLOR, programStageOfferingId: mapping?.programStageOfferingId || '', requirementIds: section.programMappings?.map((item) => item.stageCourseRequirementId) || [], cohortOfferingId: section.cohortOfferingSections?.[0]?.cohortOffering?.id || '', teacherIds: section.teachers?.map((item) => item.id) || [], studentIds: section.students?.map((item) => item.id) || [] });
    ...
    const submit = async (event: FormEvent) => {
        ...
        setError('');
        try {
            const payload = { name: form.name, code: form.code, courseId: form.courseId, academicCycleId: form.academicCycleId, defaultRoomId: form.defaultRoomId || undefined, color: form.color, teacherIds: form.teacherIds, programMappings: form.programStageOfferingId ? form.requirementIds.map((stageCourseRequirementId) => ({ programStageOfferingId: form.programStageOfferingId, stageCourseRequirementId })) : [], ...(affectedSchedules.length ? { scheduleTeacherResolution: scheduleResolution === 'DELETE' ? { action: 'DELETE' as const } : { action: 'MOVE' as const, teacherId: scheduleTeacherId } } : {}) };
